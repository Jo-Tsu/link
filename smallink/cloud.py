"""Smallink Cloud client: sign-in and managed one-click connectors.
...
def available(config: Config) -> bool:
    """Whether this build has a complete Smallink-owned cloud configuration."""
    return all(
        (
            config.cloud_base_url,
            config.cloud_auth_domain,
...
def begin_login(config: Config) -> dict[str, Any]:
    ...
    as "Firefox can't connect to 127.0.0.1:42871" right after Auth0 finished.
    """
    if not available(config):
        return {
            "ok": False,
            "error": "Smallink Cloud is not configured in this build",
        }

    verifier = _b64url(_secrets.token_bytes(48))
    challenge = _b64url(hashlib.sha256(verifier.encode()).digest())
    port = os.environ.get("LINK_PORT") or config.port
...
# --- telemetry (Phase 5) ---------------------------------------------------------
# One sentence: which agent type was started and when — nothing else. Signed-in
...
def begin_managed_connect(
    ...
    *,
    ...
    # the packaged app runs the sidecar on a random port, not 42871.
    port = os.environ.get("LINK_PORT") or config.port
    try:
        resp = httpx.post(
