...
Smallink) wrap their existing agent builders (exact prompts preserved), and markdown manifests
...
from ..agents.link import LINK_CAPABILITIES, link_agent
from .manifest import PersonaManifest, load_manifest_file

DEFAULT_PERSONA_ID = "link"
...
class PersonaRegistry:
    ...
    def _load_builtin(self, builtin_dir: Optional[str | Path]) -> None:
        # Core surfaces keep their exact prompts via the existing builders. Smallink (the default)
        # leads; Chat is hidden from the picker by default (Smallink covers quick Q&A) — recoverable
    ...
    def is_enabled(self, persona_id: str) -> bool:
        ...
        # 2026-07-09): a fresh install is Agent-only, everything else is opt-in from
    ...
    def default_id(self) -> str:
        # The configured default if it's enabled, else link if present, else any enabled one.
