...
class SQLiteSensoryStore:
    ...
    def matching_ids(
        ...
        *,
        ...
        """Return every record id matching a deletion boundary, without pagination."""
        normalized_ids = list(
            dict.fromkeys(str(record_id) for record_id in (record_ids or []) if record_id)
        )
        if len(normalized_ids) > 500:
            ...
            for start in range(0, len(normalized_ids), 500):
                matched.extend(
                    self.matching_ids(
                        record_ids=normalized_ids[start : start + 500],
                        source_type=source_type,
                        before=before,
                    )
                )
        ...
        if normalized_ids:
            clauses.append(
                "record_id IN (" + ",".join("?" for _ in normalized_ids) + ")"
            )
