"""Standalone FakeSlack runner — drive the live dev app against a fake Slack.
    ...
    python -m link.testing.fake_slack --port 8910
...
``link-server`` and connecting Slack with any fake ``xoxb-``/``xapp-`` tokens.
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(prog="smallink.testing.fake_slack")
    parser.add_argument(
    ...
    print(
        "  # then start link-server and connect Slack with any xoxb-/xapp- tokens"
    )
    print()
    ...
    print(
        ...
        '"message_ts":"1700000001.000001","action_id":"link_0","value":"..."}\''
    )
    print(
