...
def explorer_tools(
    *,
    ...
    def explore(task: str) -> dict:
        ...
        child_run_id = (
            run_observer.start(
                agent_role="explorer", model=model, input_value={"task": task}
            )
        ...
        async def _run() -> tuple[str, str]:
            ...
            async for event in engine.run(task):
                if run_observer is not None:
                    run_observer.event(
                        child_run_id, event.type.value, event.data
                    )
        ...
        try:
            ...
            if run_observer is not None:
                run_observer.finish(
                    child_run_id, status="failed", error=str(exc)
                )
            raise
        ...
        if run_observer is not None:
            run_observer.finish(
                ...
                output={"report": report, "engine_status": status},
