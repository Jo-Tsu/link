...
Every extracted fact is written as a governance candidate. It is NOT injected into prompts or
shown as confirmed memory until the user accepts, edits, or merges it. The pipeline never changes
raw content; governance status and task claims make repeated runs idempotent.
...
class MemoryPipeline:
    ...
    def _ensure_allowed(self, record: "SensoryRecord") -> None:
        ...
        if sensitivity == "secret" or _SECRET_PATTERN.search(record.normalized_content):
    ...
    def _extract(self, record: "SensoryRecord") -> list[dict[str, Any]]:
        ...
        if len(content) > _MAX_GOVERNANCE_CHARS:
            digest = hashlib.sha256(content.encode("utf-8")).hexdigest()
            content = (
                content[:_MAX_GOVERNANCE_CHARS]
                + f"\n\n[TRUNCATED source_sha256={digest} original_characters={len(content)}]"
            )
        messages = [
