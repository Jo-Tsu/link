...
def connector_list(secrets: SecretStore) -> list[dict[str, Any]]:
    ...
    for d in list_descriptors():
        ...
        if d.name == "minem":
            # Runtime inspection is deliberately passive: opening Settings must
            # not launch MineM. The explicit Connect action below owns launch.
            from .minem_client import get_minem_client

            entry.update(
                {
                    **get_minem_client().inspect(),
                    "app_version": profile.get("app_version") or "",
...
def connect_connector(
    ...
    *,
    ...
    if d.auth == "local_app":
        ...
        result = get_minem_client().connect()
        if not result.get("ok"):
            return result
        profile = {
            "type": "local_app",
            "enabled": True,
            "account": result.get("account") or "MineM local",
