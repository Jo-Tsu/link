...
the Smallink Cloud app requests. Overclaiming here is a product bug.
...
ABOUT: dict[str, str] = {
    "telegram": "Chat with your agent from Telegram. Messages to your bot "
    "reach the agent and replies come back to the same chat — only senders on "
    "your allow-list get through.",
    "slack": "Bring your agent into Slack: mention it in a channel or DM it, "
    ...
    "status. One click installs the Smallink GitHub App on the repositories "
    ...
    "minem": "Use MineM as a local material source for Smallink agents. "
    "Smallink starts MineM through its installed CLI, then uses a curated "
    "read-only tool set to search reports, pages, resources, versions, and lineage.",
    "codex": "Import your local Codex conversation history as memory source data. "
    "Smallink reads the rollout session files Codex writes under ~/.codex/sessions and "
    "ingests each message into the raw source pool, where the memory pipeline can distill it.",
    "traex": "Import your local TraeX and TRAE CLI conversation history as memory source data. "
    "Smallink reads user-owned rollout sessions under ~/.trae/cli/sessions and excludes "
    "internal subagent threads before they enter the raw source pool.",
}
...
ACCESS: dict[str, list[str]] = {
    ...
    "minem": [
        "Starts the MineM app through MineM's installed command-line interface.",
        "Searches and reads reports, pages, resources, versions, and lineage.",
        "Never writes to MineM and never reads MineM's database directly.",
    ],
    "codex": [
        "Reads your local Codex session files under ~/.codex/sessions.",
        "Imports each conversation message into Smallink's raw source pool.",
        "Read-only and fully local — never uploads or modifies your Codex data.",
    ],
    "traex": [
        "Reads your local TRAE CLI session files under ~/.trae/cli/sessions.",
        "Imports user conversations and excludes internal subagent sessions.",
        "Read-only and fully local — never uploads or modifies your TRAE data.",
    ],
}
