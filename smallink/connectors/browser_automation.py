"""Playwright-backed browser automation tools for Smallink.
...
class _BrowserController:
    def __init__(self) -> None:
        ...
        self._executor = ThreadPoolExecutor(
            max_workers=1, thread_name_prefix="agent-browser"
        )
    ...
    def browser_screenshot(path: str = "") -> dict[str, Any]:
        def run(page):
            out = (
                ...
                if path
                else Path(tempfile.gettempdir()) / "agent-browser-screenshot.png"
            )
            out = out.resolve()
