"""Smallink-only connector tools for first-party integrations.
...
def make_integration_tools(
    ...
    *,
    ...
    # Modular domain tools (extracted from this file)
    tools.extend(make_minem_tools())
    # Keep the request callable injectable through this legacy composition seam.
    # Existing embedders/tests patch integration_tools._request; passing it explicitly
    # preserves that contract while the implementation lives in tools_domain/github.py.
    tools.extend(make_github_tools(secrets, roots=roots, request_fn=_request))

    def browser_read_url(url: str, max_chars: int = 20000) -> dict[str, Any]:
        if not url.lower().startswith(("http://", "https://")):
            return {"error": "url must start with http:// or https://"}
        out = _request("GET", url, headers={"User-Agent": "link/0.1 (+connector)"})
        if "error" in out:
            return out
        data = out["data"]
