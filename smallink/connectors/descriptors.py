...
class ConnectorDescriptor:
    ...
    icon: str
    blurb: str
    auth: str  # "bot_token" | "oauth" | "token" | "api_token" | "local_app" | "none"
    two_way: bool
    fields: list[Field]
    ...
    risk_notice: str = ""
    # One-click managed OAuth via Smallink Cloud (requires cloud sign-in).
...
DESCRIPTORS: list[ConnectorDescriptor] = [
    ...
    ConnectorDescriptor(
        ...
        title="Slack",
        icon="💬",
        blurb="Two-way messaging — one-click via Smallink Cloud, or a manual Slack app (Socket Mode).",
        auth="socket_app",
        two_way=True,
        channels=True,
        brand_color="#611f69",
    ...
    ConnectorDescriptor(
        ...
        fields=[],
        instructions=[
            "No setup required. Browser tools are available to Smallink sessions."
        ],
        available=True,
    ),
    ConnectorDescriptor(
    ...
    ConnectorDescriptor(
        ...
        instructions=[
            "One click connects via Smallink Cloud (recommended).",
    ...
    ConnectorDescriptor(
        ...
        fields=[],
        instructions=[
            "Install MineM in Applications.",
            "Choose Launch and connect. Smallink uses MineM's public CLI and never reads its database directly.",
        ],
        brand_color="#00529b",
        logo="minem",
        aliases=("materials", "reports", "pages", "assets", "素材", "报告"),
    ),
    ConnectorDescriptor(
        name="codex",
        title="Codex",
        icon="⌘",
        blurb="Import your local Codex conversation history as memory source data.",
        auth="local_app",
        two_way=False,
        fields=[],
        instructions=[
            "Codex Desktop/CLI stores sessions locally under ~/.codex/sessions.",
            "Choose Connect to import recent conversations. Smallink only reads the local rollout files.",
        ],
        brand_color="#000000",
        logo="codex",
        aliases=("codex", "chatgpt", "conversations", "history", "对话", "历史"),
    ),
    ConnectorDescriptor(
        name="traex",
        title="TRAE CLI",
        icon="T",
        blurb="Import your local TraeX and TRAE CLI conversations as memory source data.",
        auth="local_app",
        two_way=False,
        fields=[],
        instructions=[
            "TRAE CLI stores sessions locally under ~/.trae/cli/sessions.",
            "Choose Connect to import user conversations. Internal subagent sessions are excluded.",
        ],
        brand_color="#0052d9",
        logo="traex",
        aliases=("trae", "traex", "trae cli", "conversation history", "对话", "历史"),
    ),
    ConnectorDescriptor(
        name="pagerduty",
        title="PagerDuty",
...
# entirely (see packaging/link-server.spec). When the package is absent this is a no-op.
try:
