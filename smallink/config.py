...
Workspace: <workspace>/.link/config.toml   (overrides global)
...
class Config:
    ...
    # Smallink Cloud is optional and disabled until an operator supplies a complete Smallink-owned
    # deployment. A fresh build must never contact an upstream product's cloud by accident.
    cloud_base_url: str = ""
...
def workspace_allowed_commands(workspace: str | Path) -> list[str]:
    ...
    path = Path(workspace).expanduser() / ".link" / "config.toml"
...
def load_config(
    ...
    *,
    ...
    if workspace:
        w = Path(workspace).expanduser() / ".link" / "config.toml"
        if w.is_file():
