...
def capabilities_for(model: str) -> ModelCapabilities:
    ...
    # TRAE CLI relay: the underlying models (Seed, Claude, GPT, DeepSeek etc.) are capable
    # but tool calling is text-based (parsed from assistant output), so disable parallel.
    if provider == "trae" or name.startswith(("seed-", "doubao-seed")):
        return ModelCapabilities(
            tools=True, vision=True, pdf=False, parallel_tool_calls=False, streaming=True
        )
