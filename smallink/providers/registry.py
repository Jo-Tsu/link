...
from .trae_provider import TraeProvider
...
def _build_trae(profile: dict[str, Any], secrets: Any) -> ProviderClient:
    model = ((profile or {}).get("model") or "").strip() or None
    return TraeProvider(model=model)
...
DESCRIPTORS: list[ProviderDescriptor] = [
    ...
    ProviderDescriptor(
        name="trae",
        title="TRAE CLI (local agent relay)",
        needs_key=False,
        fields=[
            ProviderField(
                "model",
                ...
                secret=False,
                required=False,
                placeholder="Seed-2.1-Turbo",
                help="Model to use when none is specified. Run `traex models` to see available options.",
            ),
        ],
        build=_build_trae,
        recommended_model="Seed-2.1-Turbo",
        blurb="Routes inference through TRAE CLI's exec subprocess. No API key needed — uses your TRAE SSO session.",
    ),
]
...
def verify_provider_key(
    ...
    *,
    ...
    """
    if name == "trae":
        from .trae_provider import probe_trae_status
        return probe_trae_status()
