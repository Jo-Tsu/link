...
class SessionManager:
    def __init__(
        ...
        *,
        ...
        if data_dir is not None:
            ...
            base = Path(self.default_workspace) / ".link"
        else:
            base = state_dir()
    ...
    def get_engine(
        ...
        *,
        ...
        if ag.needs_workspace and (not ws or not Path(ws).is_dir()):
            # Knowledge roles start without a selected folder, so provision a per-session
            # scratch directory. Code roles still require a real repository; Chat needs none.
        ...
        if is_new_session:
            ...
            proj = self.session_store.get_project_by_workspace(ws)
            if proj:
                self.session_store.set_session_project(session_id, proj.project_id)
        return engine
    ...
    def set_persona_enabled(self, persona_id: str, enabled: bool) -> dict[str, Any]:
        ...
        non-internal) sessions — disable means "put this agent and its history away", so
    ...
    def session_connections_view(
        ...
        the owner's 2026-07-03 finding (a fresh Project Manager session rendered link's view).
        """
        persona = self._persona_of(session_id, persona_id)
        entry = self.personas.get(persona)
        manifest = entry.manifest if entry else None
    ...
    def list_connectors(self) -> list[dict[str, Any]]:
        ...
        managed_available = cloud.available(load_config())
        for c in connectors:
            # Managed OAuth is a Smallink service capability, not an inherited default. The UI
            # must only advertise it when a complete Smallink-owned deployment is configured.
            c["managed"] = bool(c.get("managed") and managed_available)
    ...
    def pick_native_folder(self) -> dict[str, Any]:
        ...
        if sys.platform == "darwin":
            cmd = [
                ...
                "-e",
                'POSIX path of (choose folder with prompt "Give the agent access to a folder")',
            ]
        elif sys.platform == "win32":
            ...
            ps = (
                ...
                "$f.Description = 'Give the agent access to a folder'; "
    ...
    def pick_native_skill_archive(self) -> dict[str, Any]:
        """Open a local .zip picker for browser-based Skill imports."""
        import subprocess
        import sys
        ...
        if sys.platform == "darwin":
            cmd = [
                ...
                'tell application "System Events" to activate',
                "-e",
                'POSIX path of (choose file with prompt "Choose a Skill .zip package" '
            ...
            ps = (
                ...
                "$f.Title = 'Choose a Skill .zip package'; "
            ...
            cmd = [
                "zenity",
                "--file-selection",
                "--title=Choose a Skill .zip package",
    ...
    async def refresh_gateway(self) -> list[str]:
        ...
        print(f"[smallink] messaging gateway reloaded: {', '.join(started) or 'none'}")
        return started

    async def _build_and_start_gateway(self) -> list[str]:
        # Keyring calls are synchronous on every supported platform and macOS Keychain can wait
        # for user interaction. Keep them off the event loop so HTTP/WS startup remains responsive.
        settings = await asyncio.to_thread(load_settings, self.secrets)
        ...
        for platform, st in settings.items():
            ...
            profile = await asyncio.to_thread(
                self.secrets.get, f"{platform}:default"
            ) or {}
    ...
    def _resolve_inbox_reply(self, event) -> bool:
        ...
        message carried an `[link:<id>]` token (so it's consumed here, not routed as a new turn) —
    ...
    async def _route_mention(self, event, ms: MessageSource, subs) -> None:
        """@Smallink tagged in a channel. A subscribed (user-connected) agent owns the channel
        and must answer; otherwise the per-thread agent session handles it — spawned on the
        ...
        if subs:
            # The user connected a agent to this channel — it answers tags; no spawn.
            msg = (
    ...
    async def _spawn_mention_session(
        ...
        """First tag in a thread: a NEW visible agent session that owns the thread. Its
    ...
    def _runtime_outcome(self, session_id: str) -> tuple[Optional[str], Optional[str]]:
        """Return the latest durable runtime outcome for a session."""
    ...
    def finalize_manual_run(self, task_id: str, run_id: str) -> dict[str, Any]:
        ...
        if run.status == "running":
            ...
            notice = next(
                (
                    m
                    for m in reversed(record.messages if record else [])
                    if m.get("role") == "notice" and m.get("kind") in {"error", "interrupted"}
                ),
                None,
            )
            succeeded = runtime_status == "completed" or (runtime_status is None and notice is None)
    ...
    # -- session roots (orphan Smallink: scratch + added folders) ------------------
    ...
    def runtime_task(self, task_id: str) -> Optional[dict[str, Any]]:
        try:
        ...
        return {
            **task.to_dict(),
            "runs": [
                run.to_dict() for run in self.runtime_store.list_task_runs(task_id)
            ],
        }

    def session_runtime_task(self, session_id: str) -> Optional[dict[str, Any]]:
        task = self.runtime_store.get_task_by_session(session_id)
        return self.runtime_task(task.task_id) if task else None

    def runtime_task_run(self, task_run_id: str) -> Optional[dict[str, Any]]:
        try:
            run = self.runtime_store.get_task_run(task_run_id)
        except KeyError:
            return None
        return {
            **run.to_dict(),
            "agent_runs": [
                agent_run.to_dict()
                for agent_run in self.runtime_store.list_agent_runs(task_run_id)
            ],
        }

    def runtime_agent_run(self, agent_run_id: str) -> Optional[dict[str, Any]]:
        try:
            run = self.runtime_store.get_agent_run(agent_run_id)
        except KeyError:
            return None
        return run.to_dict()

    def runtime_agent_events(self, agent_run_id: str) -> Optional[list[dict[str, Any]]]:
        try:
            self.runtime_store.get_agent_run(agent_run_id)
        except KeyError:
            return None
        return [
    ...
    def _skill_loader(self, workspace: Optional[str] = None) -> SkillLoader:
        resolved = self.resolve_workspace(workspace) if workspace else self.default_workspace
        return SkillLoader(
            default_skill_sources(resolved, state_root=self._data_base)
        )

    def _skill_store(self, workspace: Optional[str] = None) -> SkillStore:
        resolved = self.resolve_workspace(workspace) if workspace else self.default_workspace
    ...
    def list_skills(self, workspace: Optional[str] = None) -> dict[str, Any]:
        ...
        return {"skills": loader.inventory(), "summary": loader.summary()}

    def skill_detail(
        self, skill_id: str, workspace: Optional[str] = None
    ) -> Optional[dict[str, Any]]:
        return self._skill_loader(workspace).detail(skill_id)

    def create_skill(self, body: dict[str, Any]) -> dict[str, Any]:
        skill = self._skill_store(body.get("workspace")).create(
            name=body.get("name", ""),
            description=body.get("description", ""),
