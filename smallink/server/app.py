...
def create_app(manager: SessionManager) -> FastAPI:
    ...
    async def lifespan(_app: FastAPI):
        try:
            ...
            if live:
                print(f"[link] messaging gateway live: {', '.join(live)}")
    ...
    app = FastAPI(title="link", version="0.0.0", lifespan=lifespan)
    ...
    def cloud_gallery_detail(slug: str) -> dict[str, Any]:
        """Solo page for one gallery agent: publisher pitch + capabilities
    ...
    @app.post("/v1/skills/pick-archive")
    ...
    async def sync_codex(body: dict | None = None) -> dict[str, Any]:
        ...
        # is offloaded off the event loop. `limit_sessions` caps how many newest sessions to read
        # (omit for all). Idempotent: re-syncing reports records_ingested=0.
        body = body or {}
    ...
    @app.post("/v1/connectors/traex/sync")
    async def sync_traex(body: dict | None = None) -> dict[str, Any]:
        # Read user-owned TRAE CLI rollouts from the granted ~/.trae folder. The reader excludes
        # subagent threads before ingesting turn-level sensory records.
        body = body or {}
        raw = body.get("limit_sessions")
        limit = int(raw) if raw is not None else None
    ...
    @app.get("/v1/sessions/{session_id}/task")
    ...
    # -- Smallink Cloud: sign-in + managed one-click connect ---------------
    ...
    @app.post("/v1/settings/model-purposes")
    def settings_set_model_purposes(body: dict) -> dict[str, Any]:
        body = body or {}
        return manager.set_model_purposes(body.get("model", ""), body.get("purposes") or [])
