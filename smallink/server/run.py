"""Launch the server with uvicorn. Used by the desktop GUI sidecar and `smallink-server`."""
...
def _exit_when_orphaned() -> None:
    """When launched as a desktop sidecar (`LINK_EXIT_WITH_PARENT=1`), exit if the parent
    process dies — even on an abrupt kill (e.g. the Tauri dev watcher restarting the app, or a
    crash) that skips the shell's graceful child-kill. Standalone `smallink-server` runs are
    unaffected.

    The GUI passes its own PID in `LINK_PARENT_PID`. Watching that explicit PID (not
    ...
    *grandchild* of the GUI — the bootloader sits in between, so getppid() points at the
    ...
    """
    if os.environ.get("LINK_EXIT_WITH_PARENT") != "1":
        return
    import threading

    try:
        parent = int(os.environ.get("LINK_PARENT_PID") or 0)
    except ValueError:
        parent = 0
...
def main(argv=None) -> None:
    ...
    parser = argparse.ArgumentParser(prog="smallink-server")
    ...
    # a random free port (to coexist with a hand-run server on 42871), so the
    # managed-connect redirect must follow the real port, not the 42871 default.
    os.environ["LINK_PORT"] = str(args.port)
