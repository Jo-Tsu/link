...
Layout under a base dir (default `~/.config/link/`):
  link.db                  SQLite index: sessions(id → project, title, n_msgs), workspaces, memory
...
class ConversationStore:
    ...
    def _backfill_project_ids(self) -> None:
        """Repair sessions created in a project before their first database checkpoint.

        Older clients identified a project session by workspace while the session row did not yet
        exist, so the attempted UPDATE was a no-op. The workspace path is unique per project and is
        therefore a safe, deterministic key for this metadata backfill.
        """
        with self._lock:
            self._conn.execute(
                """
                ...
                SET project_id = (
                    ...
                    FROM projects
                    WHERE projects.workspace_path = sessions.workspace
                )
        ...
        with self._lock:
            ...
            self._conn.execute(
                ...
                ON CONFLICT(session_id) DO UPDATE SET
                    ...
                    grants = excluded.grants, project_id = COALESCE(excluded.project_id, sessions.project_id),
    ...
    def list_projects(self, *, status: Optional[str] = None) -> list[ProjectRecord]:
        with self._lock:
            if status:
                rows = self._conn.execute(
                    "SELECT * FROM projects WHERE status = ? ORDER BY pinned DESC, sort_order ASC, updated_at DESC",
                    (status,),
                ).fetchall()
            else:
                rows = self._conn.execute(
                    "SELECT * FROM projects ORDER BY pinned DESC, sort_order ASC, updated_at DESC"
                ).fetchall()
        return [
            ProjectRecord(
    ...
    def get_project_by_workspace(self, workspace_path: str) -> Optional[ProjectRecord]:
        with self._lock:
            row = self._conn.execute(
    ...
    def update_project(self, project_id: str, **kwargs: Any) -> bool:
        allowed = {"name", "icon", "description", "status", "default_agent", "default_model", "pinned", "sort_order"}
        sets, params = [], []
    ...
    def project_session_count(self, project_id: str) -> int:
        with self._lock:
            row = self._conn.execute(
