...
# Embeds the item id in a delivered Smallink message.
_ID_TOKEN = re.compile(r"\[link:([0-9a-f]{6,})\]")
...
def resolve_from_reply(
    ...
    Looks for the ``[link:<id>]`` token and an allow/deny intent; falls back to treating
    the whole message as a free-text answer. ``resolve(item_id, resolution)`` is the
    InboxStore.resolve. Returns the resolve() result, or None if no item id was found."""
