...
> 状态：第一阶段已实现并完成真实 MineM 联调；作为底层连接适配设计继续有效，产品入口已由应用中心方案接管  
> 更新日期：2026-07-30  
> 范围：只打通同一台 Mac 上的 Smallink 客户端与 MineM 客户端，不复制 MineM 数据库，不建设云端中转

> 后续事实源：[应用中心与 MineM PRD](./smallink-app-center-minem-prd.md)、[应用中心与 MineM 技术设计](./smallink-app-center-minem-technical-design.md)。本文保留第一阶段 CLI、只读工具和安全边界，不再定义 MineM 的最终产品入口。
