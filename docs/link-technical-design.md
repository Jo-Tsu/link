...
FastAPI lifespan 只负责建立核心服务边界。消息网关通过后台任务恢复，不能再由
`manager.start_gateway()` 阻塞 ASGI startup；关闭时取消未完成任务并统一执行
`manager.aclose()`。客户端以 `service -> data -> session` 三个阶段驱动启动界面：先等待
健康检查，再并行读取会话、项目、设置和 Persona，最后恢复最近会话。

### 41.2 请求所有权

`App` 是启动基础数据的唯一所有者。它将 Persona、导航布局和侧栏数量配置作为属性传给
`Sidebar`，侧栏仅在独立挂载或数据缺失时使用兼容性回退请求。会话与项目刷新使用共享的
in-flight Promise 合并并发调用；新旧响应相同时复用原状态对象，避免无意义重渲染。

### 41.3 刷新与交互反馈

- 会话与项目使用 30 秒可见窗口兜底轮询，窗口聚焦且数据超过 10 秒未更新时补拉。
- Inbox 与 unattended 状态使用 15 秒可见窗口轮询，并在重新聚焦时刷新。
- 已有会话切换期间显示固定结构的消息骨架，防止旧内容残留和布局跳动。
- 启动界面显示当前阶段、进度轨和辅助说明；服务异常时继续复用明确的恢复与重试状态。

### 41.4 本轮验证

- 前端生产构建通过，共转换 422 个模块。
- 前端测试 23 个文件、106 项全部通过。
- 服务端启动、项目会话和连接器状态相关测试 71 项全部通过。
- 安装包 `Smallink_0.2.0_aarch64.dmg` 构建成功，并在 `/Applications/Smallink.app` 完成真实启动验收。

## 42. 产物链接解析与打开

### 42.1 链接规范化

ReactMarkdown 会对中文和空格等非 ASCII 路径执行 URL 编码。`Markdown` 在渲染产物卡片
前最多执行两次安全 `decodeURIComponent`，并统一路径分隔符。`artifact:` 链接始终进入
产物链路；没有协议但扩展名属于受支持产物类型的相对文件链接也按产物处理。HTTP、HTTPS、
邮件和页内锚点继续走普通链接逻辑。

### 42.2 查找与安全边界

`RightRail` 使用规范化后的相对路径、绝对路径和文件名匹配当前会话的产物列表。服务端
`_artifact_target()` 同时尝试原始值和最多两次 URL 解码值，每个候选仍必须通过工作空间
`relative_to(root)` 校验，因此兼容编码路径不会放宽目录逃逸保护。

### 42.3 点击反馈

产物深链监听器在右侧栏隐藏时仍保持注册。点击后先创建最小产物记录并立即进入预览状态，
随后刷新产物列表并替换为完整元数据。读取请求失败时写入明确错误状态，不能把 `content`
重置为空并永久显示“加载中”。

### 42.4 验证

- 中文 `artifact:` 路径能够按原文显示和派发。
- 遗漏 `artifact:` 的相对 Markdown 产物链接能够渲染为产物卡片。
- 右侧栏隐藏时点击 URL 编码的中文产物链接，重新展开后能够读取真实文件。
- 服务端能够读取 URL 编码的中文文件，同时继续拒绝目录逃逸路径。

## 43. 应用中心与应用运行框架

### 43.1 架构定位

应用中心是 `connectors` 之上的聚合模块。连接器解决“如何连接和调用”，应用解决“如何为用户
提供入口、项目、智能体、能力、数据和记忆”。MineM 是第一个内置应用，现有 MineM CLI 客户端
继续作为底层 Adapter，不复制第二套传输实现。
...
```text
apps.registry       应用清单和能力校验
apps.instances      本机安装、启用、运行和兼容状态
apps.projects       系统应用项目的幂等创建与归属
apps.runtime        能力装配、执行、审批、幂等和恢复
apps.asset_refs     外部素材引用和读取快照
apps.memory_policy  应用级候选记忆规则
apps.builtin.minem  MineM 清单、Adapter、工具、工作流和卡片映射
apps.workspace      AppHost、Workspace Bridge 和嵌入界面生命周期
```

系统项目使用 `project_type=system_app`、`owner_app_id=minem` 和
`workspace_path=NULL`，不得伪装成本地文件夹。会话仍复用统一 Conversation、TaskRun、AgentRun
和 WebSocket，不建设第二套聊天运行时。

### 43.3 数据边界

MineM 是页面、汇报、资源、版本、编排和导出文件的唯一事实源。Smallink 保存：

- 应用实例和系统项目；
- CapabilityRun、审批和运行事件；
- SensoryRecord 原始事实；
- 实际读取素材的版本快照；
- MineM 素材 ID、编号、版本和内容哈希引用；
- Smallink 产物映射；
- 候选记忆、来源和人工确认。

搜索刷新不得重复复制素材正文；相同写操作必须通过幂等键和 MineM 真实状态核验防止重复发布。

### 43.4 MineM 完整素材工作台

`system:minem` 必须挂载与受支持 MineM 版本能力对齐的完整工作台，覆盖工作台、汇报、页面、案例、
资源、故事线、导入任务、素材治理、预览、版本、编排、演讲稿和导出。实现采用双链路：

- Agent/自动化动作：`App Runtime -> MineM CLI -> MineM Service`；
- 高频界面动作：`AppHost -> Workspace Bridge -> MineM Public UI API -> MineM Service`。

MineM 在自身仓库内维护可嵌入前端 Surface 和领域 API；Smallink 不复制 `App.tsx` 或业务组件，只维护
外壳、契约、版本协商、权限、对象引用、语义事件和对话交接。两条链路必须使用同一素材 ID、版本、
事务和领域校验。工作台产生的搜索、查看、标签、编排、合并、删除、导入和导出语义事件进入
SensoryRecord，但不采集无意义鼠标轨迹。

### 43.5 迁移状态

阶段 1 先建立应用清单、实例和 `system:minem`，并迁移现有六个只读工具；阶段 2 抽取 MineM 可嵌入
工作台、Public UI API 并完成全量素材管理；阶段 3 增加智能体页面构建、发布、汇报创建和导出；
阶段 4 增加智能编排、影响预览和记忆治理。旧连接器入口只有在新应用入口、
历史迁移和完整测试通过后才能收敛。

详细数据模型、CLI 白名单、API、事件、幂等恢复和代码结构见
[应用中心与 MineM 技术设计](./smallink-app-center-minem-technical-design.md)，测试门禁见
[应用中心与 MineM 测试计划](./smallink-app-center-minem-test-plan.md)。

## 44. 2026-08-15 稳定性修复实现

### 44.1 启动和密钥库

`SessionManager._build_and_start_gateway()` 使用 `asyncio.to_thread` 读取连接器设置和密钥 Profile。
macOS Keychain、Windows Credential Locker 或 Linux Secret Service 即使等待系统交互，也不能阻塞
FastAPI 事件循环。HTTP 健康检查和 WebSocket 先恢复，连接器在后台完成重连。

### 44.2 前端 API 错误边界

`surfaces/gui/src/api.ts` 的统一请求包装器负责 Sidecar Token、非 2xx 校验和结构化 `ApiError`。
401 会发布认证失效事件，错误响应不会再被解析为 `ModelSettings`、Session 或 Project。应用启动失败
进入带重试按钮的服务异常页；React 渲染异常由 `AppErrorBoundary` 承接并保留重新加载入口。

### 44.3 原始事实保护

`_sensory_safe()` 递归识别所有层级的 `data:` URL，写入 `{omitted, characters, sha256}`，不保存
Base64 正文。第一方运行记录和通用 Ingest 根据显式凭证模式标记 `private/secret`。治理管道在调用
模型前再次拦截显式密钥，并把单条模型输入限制为 48,000 字符；截断标记包含原始字符数和 SHA-256。

### 44.4 项目与运行关系

创建 Runtime Task 时优先使用 Session 的 `project_id`；旧会话缺少关联时按规范化 workspace 查找
Project 并写入 UUID。启动时 `repair_legacy_project_ids()` 将历史路径转换为 Session 或 Project 对应的
UUID，无法解析的非法关联清为空。执行目录继续保存到 SensoryRecord 的 `project_path`，不得替代项目主键。

### 44.5 本机能力显式启用

TRAE 模型发现只有在用户配置 TRAE Provider、主动加入 `trae:*` 模型或当前默认模型属于 TRAE 时运行。
Keyless Provider 配置写入 `_enabled` 标记；仅安装本机程序不构成授权，也不改变全新用户的模型列表。

### 44.6 回归门禁

- Python：976 passed，1 skipped。
- React/Vitest：24 files，110 tests passed。
- TypeScript/Vite 生产构建通过。
- Rust `cargo check` 通过。
- Playwright：6 个现有 E2E 通过；真实 macOS Keychain 模式启动并通过 `/v1/health` 验证。
