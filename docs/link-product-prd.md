...
| 多智能体 | 已可用第一版 | Explorer 保留代码探索兼容链路；主智能体可通过统一协议委派 Researcher、Analyst、Reviewer，只读子运行拥有独立上下文、真实父子 AgentRun、事件和客户端运行树 | 增加依赖图、预算、父子取消、失败重试和跨任务调度 |
| 数据连接器与应用 | 已可用第一版 | 当前客户端只展示 MineM；已有应用清单、实例、`system:minem`、素材工作台第一版、专属会话、运行/来源沉淀；Codex、TRAE 后端同步适配器按产品策略隐藏 | 接入 MineM 完整可嵌入工作台，并扩展更多经过验证的来源和自动增量调度 |
| `sensory_records` | 已可用第一版 | 运行时和本地连接器事实进入 SQLite；支持来源、状态、服务端搜索、分页、原文和删除；治理状态可推进与重试 | 批量删除影响预览、保留策略和 Postgres 迁移 |
| AI 数据治理 | 已可用第一版 | 真实 Provider、原子任务领取、敏感策略、独立候选、失败重试、接受、编辑、合并、忽略、批量决定、批量改类型和默认关闭的增量后台调度已落地 | 冲突任务、提示词评估和治理质量指标 |
| 正式个人记忆 | 已可用第一版 | 正式记忆、版本、来源和决策事务已落地；客户端提供十种类型、候选处理、来源证据和归档恢复；active 记忆按 global/workspace/session 作用域、关键词相关性和字符预算注入，并记录使用版本 | 向量语义检索、Token 级 ContextBundle、关系和效果反馈 |
| 知识库 | 已可用第一版 | 真实知识条目、版本、切片、项目隔离、FTS5/BM25 混合词法排序、来源解释和项目智能体只读检索已落地 | 向量语义召回、文件解析扩展、重建任务和效果评估 |
| 项目空间 | 已可用第一版 | 项目页聚合会话、来源、记忆、知识和应用素材；来源可展开候选、决定、正式记忆和使用记录 | 增加项目级权限、批量治理视图和跨项目移动 |
