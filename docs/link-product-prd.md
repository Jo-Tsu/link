...
关联文档：[技术架构设计](./link-technical-design.md)、[智能体 AI 底座](./smallink-agent-ai-foundation-guide.md)、[个人记忆 PRD](./link-memory-prd.md)、[个人记忆技术方案](./link-memory-technical-design.md)、[UI 设计系统](./smallink-ui-design-system.md)
...
| 自动化 | 部分可用 | 已有定时任务、模板和运行基础 | 全部运行映射正式 TaskRun 并完善恢复 |
| Task / TaskRun / AgentRun | 部分可用 | 真实运行链已落地，当前使用 SQLite 适配器 | 迁移 Postgres，补暂停恢复和运行图 |
| 多智能体 | 可视化第一版已落地 | Explorer 已作为真实可观察子运行；客户端提供成员、协作任务、父子运行图和节点详情 | 通用委派、依赖、并行、预算和父子取消 |
| 数据连接器 | 部分可用 | 已有连接器与部分数据接入基础 | 所有来源使用统一同步任务与水位 |
| `sensory_records` | 第一阶段已贯通 | 会话输入、连接器入站、AI 输出、工具调用/结果及运行错误以不可变记录进入 SQLite；客户端提供来源、状态、项目、会话、哈希和原文查询 | Codex 历史与外部连接器全量/增量同步，记录版本和 Postgres 迁移 |
| AI 数据治理 | 待建设主链 | 产品流程、候选结构和提示词规则已确定 | 真实 Provider、真实任务、批量状态闭环 |
| 正式个人记忆 | 页面入口已落地，主链待建设 | 客户端按十种标准类型展示真实记忆数量、类型列表与详情；旧 SQLite 记忆只读兼容，未分类内容单独提示 | 正式表、版本、来源、候选确认、冲突处理与检索落地 |
