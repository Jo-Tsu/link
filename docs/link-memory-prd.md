...
> **当前进度（2026-08-20，SQLite 过渡实现）**：已贯通 `sensory_records` 原始池、真实模型 Provider、治理任务、独立候选记忆、人工单条/批量纳入或忽略、正式记忆、版本/来源/决定记录，以及 `global`、`workspace`、`session` 三种作用域的对话检索注入与使用留痕。Codex、TRAE 同步适配器仍保留在后端，当前客户端按产品策略只展示 MineM 连接器。尚未完成批量调整类型、正式记忆归档恢复、向量/混合检索、知识库、Postgres 迁移和自动治理调度。
