...
版本：1.0  
更新日期：2026-07-26  
产品阶段：Link 二次重构中的正式记忆模块  
关联文档：[Link 产品技术架构设计](./link-technical-design.md)、[Link 个人记忆模块技术方案](./link-memory-technical-design.md)
...
- 保留 Link 执行内核在任务中使用长期记忆的场景。
- 不继承旧 Mock 数据、没有真实后端支撑的页面逻辑和简单 `MemoryStore`。
- 以 Postgres 重新建设候选记忆、正式记忆、版本、来源、关系和检索能力。
- AI 负责分析和提出建议，用户拥有正式记忆的最终决定权。

> **当前进度（2026-08，SQLite 过渡实现）**：已落地记忆的**数据层管道**——通用采集入口（`POST /v1/sensory-records`）把外部原始数据写入不可变原始层，再由 `MemoryPipeline`（`POST /v1/memory/pipeline/run`，手动触发）完成**清洗（LLM 提炼）→ 标记（10 种记忆类型）→ 分层（global/workspace/session）**，产出 `status='pending'` 的记忆并保留 `source_record_id` 溯源与 `memory_history` 版本行。本期**不注入对话、不注册 agent 写工具、不改动正常对话上下文**；pending 记忆默认对 prompt 与确认视图不可见（仅 `?status=pending` 可查验）。**人工确认流、agent 检索、去重合并、真正的本地/外部数据连接器均尚未实现**，详见技术方案第 0 章。此过渡实现遵循“AI 只产出候选、用户拥有最终决定权”的原则：管道产物停在 pending，等待后续确认闭环。

## 2. 产品定位

Link 个人记忆模块用于保存用户希望长期保留，并希望未来智能体能够正确使用的个人结论。
