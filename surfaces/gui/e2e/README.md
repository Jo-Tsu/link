...
backend on :42871. Two flavors, both skip cleanly when the backend is down:
...
- **Full vertical** (`fib.spec.ts`, …) — asks a fresh Link session to produce `fib.md` and
...
- Seed data worth knowing: the pinned session "Draft the launch note" is the newest (boot-resume
  target); 7 unpinned "Weekly plan N" link sessions exercise the sidebar peek cap; two pending
  Inbox items (approval on link, question on ops) drive the Inbox filters; `acme-notes` is a
