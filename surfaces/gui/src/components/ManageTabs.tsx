...
export function ModelsTab() {
  ...
  return (
    <div>
      ...
      {info?.configured ? (
        <div className="mt-6">
          <div className={SEC_H + " mb-1.5"}>{tr("Models")}</div>
          <p className="text-[12px] text-muted mb-2.5 leading-relaxed">
            {tr("Selected models appear in the composer. The dark badge marks the default for new sessions.")}
          </p>
          <ModelChecklist
            provider={ps.sel}
        ...
        (info?.suggested_models?.length || 0) > 0 && (
          <div className="mt-6" data-testid="model-preview">
            <div className={SEC_H + " mb-1.5"}>{tr("Included models")}</div>
            <p className="text-[12px] text-muted mb-2.5 leading-relaxed">
              {tr("These curated models support agent work. Add your key above to enable them.")}
            </p>
...
function ComposerPickerCard({
  ...
  return (
    <div className="mt-6" data-testid="composer-picker">
      <div className={SEC_H + " mb-1.5"}>{tr("In the composer")}</div>
      <p className="text-[12px] text-muted mb-2.5 leading-relaxed">
        {tr("These models are available when starting a session. Add more from a provider card above.")}
      </p>
      <div className="mlist">
            ...
            <div className="mlist-row" key={id}>
              <label className="mlist-main">
                <input
                  ...
                  checked
                  disabled={isDefault}
                  title={tr(isDefault ? "The default model is always shown. Make another model the default first." : "Remove from the picker")}
  ...
  return (
    <div className="space-y-3">
      <p className="text-[12.5px] text-muted leading-relaxed">
        {tr("MCP servers provide external tools to every agent. Enabled tools remain permission-controlled, and changes apply to new sessions.")}{" "}
        <button
      ...
      {MCP_PRESETS.filter((p) => !servers.some((s) => s.name === p.name)).map((p) => (
        <div key={p.name} className={CARD + " p-3.5 flex items-center gap-3"} data-testid={`mcp-preset-${p.name}`}>
          <div className="flex-1 min-w-0">
            ...
            <div className="text-[11.5px] text-faint">{tr(p.blurb)}</div>
          </div>
          <button
...
function McpRow({
  ...
  const loadTools = async () => {
    ...
    else setToolErr(res.error || tr("Failed to connect."));
  };

  return (
    <div className={CARD + " p-3.5"}>
      <div className="flex items-center gap-3">
        <Toggle checked={server.enabled} onChange={onToggle} title={tr("Enable this server")} />
        <div className="flex-1 min-w-0">
          <div className="text-[14px] font-medium">{server.name}</div>
          <div className="text-[11.5px] text-faint">
            {server.transport} · {authorizing ? tr("Signing in…") : tr(server.status.replace("_", " "))}
            {server.tool_count != null ? ` · ${tr("{count} tools", { count: server.tool_count })}` : ""}
            {server.requires_approval ? ` · ${tr("asks first")}` : ""}
            {isOauth ? " · oauth" : ""}
          </div>
        </div>
        {isOauth &&
          (server.status === "needs_auth" ? (
            ...
            <span className="text-[12px] text-muted shrink-0">{tr("Waiting for browser…")}</span>
        ...
        <button
          ...
          disabled={busy}
        >
          {busy ? "…" : tr(tools ? "Hide tools" : "Tools")}
        </button>
      ...
      {tools && (
        <div className="mt-2.5 pt-2.5 border-t border-line flex flex-wrap gap-1.5">
          {tools.length === 0 && <div className="text-[12px] text-faint">{tr("No tools.")}</div>}
          {tools.map((t) => (
            <span
              key={t.name}
              title={t.description}
...
function AddForm({
  ...
  const save = async () => {
    ...
    try {
      ...
      onError(tr("Invalid JSON: {error}", { error: e.message }));
      return;
    }
    ...
    if (!entries || entries.length === 0) {
      onError(tr('Paste a `{ "<name>": { … } }` object or a full mcpServers block.'));
      return;
    }
  ...
  return (
    <div className="space-y-2">
      <div className="text-[12.5px] text-muted">{tr("Paste server JSON (name → config):")}</div>
      <textarea
        value={text}
  ...
  return (
    ...
    >
      <div className={SEC_H + " mb-2"}>
        {tr("Messages from senders you haven't allowed")} · {items.length}
      </div>
      <div className="space-y-2">
        {items.map((m) => (
          <div key={m.id} className="rounded-xl border border-line bg-paper p-2.5">
            <div className="flex items-center gap-2 text-[12px] text-muted">
              ...
              <span>{tr("in {place}", { place: m.chat_name || m.chat_id })}</span>
            ...
            <div className="flex items-center gap-1.5 mt-2">
              <button
                ...
                title={tr("Add the sender to the allow-list and deliver this message now")}
                onClick={() => act(m.id, "allow_deliver")}
              >
                {tr("Allow and deliver")}
              </button>
              <button
                className={BTN_BORDERED}
                data-testid={`parked-allow-${m.id}`}
                title={tr("Add the sender to the allow-list; discard this message")}
...
export function ListeningSessionsBlock({ c }: { c: Connector }) {
  ...
  return (
    <div className="border-t border-line px-3.5 py-3" data-testid={`listening-${c.name}`}>
      <div className={SEC_H + " mb-2"}>
        {tr("Sessions listening to {name} channels", { name: c.title })} · {mine.length}
      </div>
      {mine.length === 0 ? (
        <div className="text-[12px] text-faint">
          {tr("No sessions are listening yet. Open a session's Sources panel to add a channel.")}
        </div>
      ) : (
          ...
          {mine.map((s) => (
            <div className="flex items-center gap-2 text-[12.5px]" key={s.session_id + s.channel}>
              ...
              <button
                ...
                title={tr("Unsubscribe this session")}
                onClick={async () => {
...
export function AllowlistBlock({
  ...
  return (
    <div className="border-t border-line px-3.5 py-3 grid grid-cols-2 gap-5">
      <div>
        <div className={SEC_H + " mb-2"}>{tr("Allowed to message")}</div>
        <div className="flex flex-wrap gap-1.5">
          {allowedUsers.length === 0 && (
            <span className="text-[12px] text-faint">
              {tr("No one yet. Allow a recent sender.")}
            </span>
          )}
          {allowedUsers.map((u) => (
            <span
      ...
      <div>
        <div className={SEC_H + " mb-2"}>{tr("Recent senders")}</div>
        {unknownRecent.length === 0 ? (
          <div className="text-[12px] text-faint">
            {tr("None yet. Send the bot a message and the sender will appear here.")}
          </div>
        ) : (
          <div className="space-y-1.5">
            {unknownRecent.map((r) => (
  ...
  if (!c.tools?.length)
    return (
      <div className="border-t border-line px-3.5 py-3 text-[12.5px] text-muted">
        {tr("No tools for this connector yet.")}
      </div>
    );
  return (
        ...
        {c.tools.map((tool) => (
          ...
          >
            ...
            <span className="min-w-0">
              <span className="block text-[13px]">{tr(tool.label)}</span>
              <span className="block text-[11.5px] text-faint">
                {tool.name} · {tool.kind} · {tr("asks approval")}
              </span>
              <span className="block text-[11.5px] text-faint">
                {tr(tool.description)}
              </span>
...
export function ConnectSetup({
  ...
  const oneClick = async () => {
    ...
    else setError(res.error || tr("Could not start one-click connection"));
  };
  ...
  const mcpOneClick = async () => {
    ...
    if (res.ok) setWaiting(true);
    else setError(res.error || tr("Could not start connection"));
  };
    ...
    <div className="border-t border-line px-3.5 py-3 space-y-3">
      {c.mcp && !manualOnly && (
        ...
        <div className="space-y-2" data-testid="mcp-connect">
          <button className={BTN_ACCENT} onClick={mcpOneClick} disabled={waiting}>
            {waiting
              ? tr("Check your browser…")
              : tr("Connect {name} with one click", { name: c.title })}
          </button>
          {c.fields.length > 0 && (
            <div className="text-[11.5px] text-faint">{tr("or connect manually:")}</div>
          )}
        </div>
      )}
      {c.managed && (cloud === null || cloud.available) && !c.mcp && !manualOnly && (
        <div className="space-y-2" data-testid="managed-connect">
          {c.managed_paused ? (
            ...
            <>
              ...
              <div className="text-[11.5px] text-faint">
                {tr("One-click sign-in is coming soon. Connect manually below for now:")}
              </div>
            </>
          ) : cloud?.signed_in ? (
            ...
            <CloudSignInInline
              blurb={tr(
                "Sign in to connect {name} with one click, or connect manually below.",
                { name: c.title },
              )}
            />
          ) : (
      ...
      {c.fields.map((f) => (
        <label className="conn-field" key={f.key}>
          ...
          />
          {f.help && <span className="conn-field-help">{tr(f.help)}</span>}
        </label>
      ))}
      <div>
