    ...
    <div className="relative h-0 select-none">
      ...
      >
        <button
          ...
          onClick={copy}
        >
          {copied ? tr("Copied") : <Icon name="copy" size={11} />}
        </button>
        {when && (
          <span data-testid="bubble-ts" title={when.toLocaleString(locale)}>
            {when.toLocaleTimeString(locale, { hour: "numeric", minute: "2-digit" })}
          </span>
        )}
      </div>
    </div>
    ...
    <div className="thinking">
      ...
      >
        ...
        <span className={live ? "thinking-live" : undefined}>
          {tr(live ? "Thinking…" : "Thought process")}
        </span>
      </button>
      {open && (
...
function ApprovalChip({ resolved }: { resolved: ApprovalDecision | undefined }) {
  const { tr } = useI18n();
  if (resolved === "deny")
    return <span className="text-[10.5px] px-1.5 rounded-full bg-dangerSoft text-danger shrink-0">✕ {tr("declined")}</span>;
  return (
    <span
      className="text-[10.5px] px-1.5 rounded-full bg-okSoft text-ok shrink-0"
      title={resolved
        ? tr("approved · {decision}", { decision: tr(resolved.replace(/_/g, " ")) })
        : tr("approved")}
    >
      ✓ {tr("approved")}
    </span>
  ...
  return (
    <div>
      <div className="group flex items-baseline gap-2 px-2 py-0.5 rounded-lg hover:bg-paper" data-testid="turn-step">
        ...
        <LineText line={humanizeTool(tool.name, tool.args, tr)} />
        ...
        {!!tool.standingRule && (
          <span
            ...
            title={tr("Auto-allowed by this automation's standing approval: {rule}. Revoke on its Automations page.", {
              rule: tool.standingRule,
            })}
          >
            {tr("auto-allowed")}
          </span>
        ...
        {!!tool.hidden && (
          <span
            ...
            title={tr("Removed by your privacy filters before the agent saw the results — agents get no trace of these.")}
          >
            {tr("{count} hidden", { count: tool.hidden })}
          </span>
        )}
...
function TurnGroup({
  ...
  const stepsLabel = tr(nSteps === 1 ? "{count} step" : "{count} steps", { count: nSteps });
  ...
  return (
    <details className="stepgroup" open={open}>
      ...
      >
        ...
        <span>
          <span>{running ? tr("Running {steps}…", { steps: stepsLabel }) : stepsLabel}</span>
          {declined > 0 && (
            <>
              {" · "}
              <span className="text-danger" data-testid="stepgroup-declined">
                {tr("{count} declined", { count: declined })}
              </span>
            </>
          )}
          {hiddenTotal > 0 && (
            <>
              ...
              <span className="text-warnInk" data-testid="stepgroup-hidden">
                {tr("{count} hidden by your filters", { count: hiddenTotal })}
              </span>
            </>
          )}
        </span>
          ...
          {rows.map((row, i) =>
            row.type === "narr" ? (
              ...
              <div className="flex items-baseline gap-2 px-2 py-0.5" key={i} data-testid="turn-ask">
                ...
                <LineText line={humanizeAsk(row.approval.name, row.approval.args, tr)} />
                    ...
                    <div className="bubble-attachments">
                      {item.attachments.map((a, i) =>
                        a.kind === "image" ? (
                          ...
                          <span key={i} className="msg-file"><Icon name="file" size={13} /> {a.name}</span>
                        ),
                      )}
                    </div>
                  )}
          ...
          case "dirreq":
            ...
            return (
              <div className="approval-inline" key={bi}>
                ...
                <span>{tr(item.resolved === "granted" ? "Granted folder access" : "Declined folder access")}</span>
          ...
          case "planreq":
            ...
            return (
              <div className="bubble-assistant" key={bi}>
                <div className="who">{tr("proposed plan")}</div>
                <Markdown text={item.plan} />
                <div className="approval-inline">
                  <span className={"status " + (item.resolved === "approved" ? "ok" : "denied")}>
                    {item.resolved === "approved" ? "✓" : "✕"}
                  </span>
                  <span>{tr(item.resolved === "approved" ? "Plan approved" : "Sent back with feedback")}</span>
                </div>
              </div>
            );
          case "notice":
