...
export function PersonasTab({ onOpenPersona }: { onOpenPersona?: (id: string) => void }) {
  ...
  const remove = async (id: string) => {
    ...
    if (!r.ok) {
      setMsg(r.error || tr("Delete failed"));
      return;
    }
    if (r.personas) setPersonas(r.personas);
    else reload();
  ...
  const install = async () => {
    ...
    if (r.personas) setPersonas(r.personas);
    setMsg(
      tr("Installed {count} agents. Review and enable them below.", {
  ...
  return (
    <div>
      <p className="text-[12.5px] text-muted mb-3 leading-relaxed">
        {tr(
          "Enable an agent and choose whether it appears in the new-session picker. The starred agent is the default for new sessions.",
        )}
      </p>
          ...
          <div key={p.id} className="px-4 py-3">
            ...
            <div className="min-w-0 flex-1">
              <div className="text-[13.5px] font-medium flex items-center gap-1.5">
                ...
                {p.default && (
                  <span className="text-accent" title={tr("Default for new sessions")}>
                    ★
                  </span>
                )}
                {p.builtin && (
              ...
              <div className="text-[12px] text-muted truncate">{tr(p.tagline)}</div>
            </div>
            ...
            {onOpenPersona && (
              <button
                ...
                title={tr("Configure {name}", { name: p.name })}
                aria-label={tr("Configure {name}", { name: p.name })}
            ...
            {confirmOff === p.id && (
              ...
              >
                <span className="min-w-0">
                  {tr(
                    "Disabling this agent archives {count} conversations. They remain available under Show archived.",
      ...
      <div className={SEC_H + " mb-1.5"}>{tr("Add agents")}</div>
      <p className="text-[12px] text-muted mb-3 leading-relaxed">
        {tr(
          "Load an agent from a local directory or public GitHub repository. Link copies the files into managed storage so the agent stays stable when its source changes. Agent packages do not run code; they only compose vetted tools.",
        )}
      </p>
      <div className="flex items-center gap-2">
        <select className={SELECT} value={mode} onChange={(e) => setMode(e.target.value as "git" | "dir")}>
          <option value="git">{tr("GitHub URL")}</option>
          <option value="dir">{tr("Local directory")}</option>
        </select>
        <input
          className={INPUT}
        ...
        <div className="mt-4 space-y-2">
          {consent.map((c) => (
            <div key={c.id} className={CARD + " p-3.5"}>
              ...
              <div className="text-[12px] text-muted mt-0.5 mb-2">
                {tr(c.description)}
              </div>
              <div className="text-[12px] text-ink">
                {tr("Tools")}: {c.tools.join(", ") || "—"}
              </div>
              <div className="text-[12px] text-ink">
                {tr("Risk")}: {c.risk.join(", ") || tr("read")}
                {c.connectors ? ` · ${tr("connectors")}` : ""}
                {c.messaging ? ` · ${tr("messaging")}` : ""}
              </div>
              <div className="text-[12px] text-faint mt-1">
                {tr("Recommended mode: {mode}. Enable this agent above to use it.", {
                  mode: tr(c.recommended_mode),
                })}
              </div>
            </div>
          ))}
