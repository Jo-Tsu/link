...
export function AccessSection({
  ...
  const addChannel = async () => {
    ...
    if (!r.ok) {
      setAddErr(r.error || tr("Couldn’t add that channel."));
      return;
    }
    setAddErr(null);
    setDraft("");
  ...
  const folderPart = projectScoped
    ...
    : roots.length > 0
      ? tr(roots.length === 1 ? "{count} folder" : "{count} folders", { count: roots.length })
      : null;
  ...
  return (
    <section className="rail-section" ref={rootEl} data-testid="access-section">
      ...
      {open && (
        <div className="rail-section-body" role="region" aria-label={tr("Session access")}>
          {connectFor ? (
            <ConnectInline
              c={connectFor}
              cloud={cloud}
            ...
            <div className="space-y-4">
              ...
              <div>
                <div className={`${SEC_H} mb-1.5`}>{tr("Sources")}</div>
                {connected.length === 0 && (
                  <div className="text-[12px] text-faint py-0.5">
                    {tr("No connectors enabled for this session.")}
                  </div>
                )}
                    ...
                    <div className="flex items-center gap-2 py-1" key={c.connector}>
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {byName[c.connector]?.channels && (
                          ...
                          >
                            {tr("Channels")} · {channelsOf(c.connector).length}
                      ...
                      <Toggle
                        ...
                        title={tr("Enabled for this session — tap to mute here")}
                      />
                    </div>
                  ))}
                </div>
                {connected.length > 0 && (
                  <p className="text-[10.5px] text-faint mt-1 leading-snug">
                    {tr("Turning it off mutes the source for this session only. The connector stays connected.")}
                  </p>
                )}
                  ...
                  <div className="mt-1.5">
                    ...
                    {results.length === 0 && (
                      ...
                      <div className="text-[11.5px] text-faint mt-1.5 px-0.5">
                        {tr("No match. Open the Connectors page below to browse all options.")}
                      </div>
                    )}
              ...
              {recommended.length > 0 && (
                <div>
                  <div className={`${SEC_H} mb-1.5`}>{tr("Recommended")}</div>
                  <div className="space-y-1">
                    {recommended.map((r) => (
                      <div className="flex items-center gap-2 py-1" key={r.connector}>
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-1.5 text-[12.5px] font-medium leading-tight">
                            ...
                            {r.tier === "core" && <span className={TAG_CORE}>{tr("Core")}</span>}
                          </div>
              ...
              <div data-testid="drawer-directories">
                <div className={`${SEC_H} mb-1.5`}>{tr("Folders")}</div>
...
function ConnectInline({
  ...
  return (
    <div>
      ...
      >
        <Icon name="arrowLeft" size={13} /> {tr("Connect {name}", { name: c.title })}
      </button>
      {c.blurb && <p className="text-[12px] text-muted mb-1 leading-relaxed">{tr(c.blurb)}</p>}
      <div className="-mx-2">
      ...
      <p className="text-[10.5px] text-faint mt-2 leading-snug">
        {tr("Connecting makes {name} available to all agents. The toggle in this list controls only this session.", {
          name: c.title,
        })}
      </p>
    </div>
  ...
  return (
    <div>
      <button
        ...
        aria-label={tr("Back to sources")}
      >
        <Icon name="arrowLeft" size={13} /> {tr("{name} channels", { name: label })}
      </button>
      <div className={`${SEC_H} mb-1.5`}>{tr("Subscribed channels")} · {channels.length}</div>
      {channels.length === 0 ? (
        <div className="text-[12px] text-faint py-0.5">
          {tr("Not listening to any {name} channel yet.", { name: label })}
        </div>
      ) : (
        <div className="space-y-1">
          {channels.map((s) => (
            <div className="flex items-center gap-1.5 py-1" key={s.channel}>
              ...
              {s.collision && (
                <span
                  ...
                  title={tr("This channel is also this session’s inbox route, so inbound and outbound traffic overlap.")}
                >
                  ⚠
                </span>
              )}
      ...
      <div className={`${SEC_H} mt-3 mb-1.5`}>{tr("Add a channel")}</div>
      ...
      <p className="text-[10.5px] text-faint mt-1.5 leading-snug">
        {tr("The agent receives messages posted to these channels. Removing one stops this session from listening; the connector stays connected.")}
      </p>
    </div>
  );
}
