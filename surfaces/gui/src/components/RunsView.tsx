...
export function RunsView({
  ...
  const selectedTaskVersion = tasks.find(
  ...
  const selectedAgentResult = readableResult(selectedAgent?.output);
  const hasStructuredResult =
    selectedAgent?.output != null && typeof selectedAgent.output === "object";
              ...
              <div className="grid grid-cols-1 min-[1280px]:grid-cols-[minmax(0,1fr)_minmax(260px,0.7fr)] gap-5">
                ...
                <section className="min-w-0">
                  ...
                  <div className="border border-line rounded-lg bg-panel p-3">
                    ...
                    <div className="whitespace-pre-wrap break-words text-[12.5px] leading-relaxed text-ink max-h-[360px] overflow-auto hairline-scroll">
                      {selectedAgentResult || t("runs.noOutput")}
                    </div>
                    {hasStructuredResult && (
                      <details className="mt-3 border-t border-line pt-2 text-[11.5px] text-muted">
                        <summary className="cursor-pointer select-none">{t("runs.rawDetails")}</summary>
                        <pre className="mt-2 mb-0 whitespace-pre-wrap break-words font-mono max-h-[240px] overflow-auto hairline-scroll">
                          {formatValue(selectedAgent?.output)}
                        </pre>
                      </details>
                    )}
                  </div>
...
function readableResult(value: any): string {
  if (value == null || typeof value === "string") return value || "";
  if (typeof value !== "object") return String(value);
  for (const key of ["text", "result", "result_text", "summary", "message", "content"]) {
