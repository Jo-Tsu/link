  ...
  return (
    <div data-testid="inbox-configure">
      ...
      <div className="mt-6" data-testid="unrouted-section">
        <h3 className="text-[14px] font-semibold mb-1">{tr("Unrouted")}</h3>
        <p className="text-[12.5px] text-muted mb-3">
          {tr("Messages and background tasks with no destination appear here instead of disappearing silently.")}
        </p>
        <UnroutedTable />
      </div>
    </div>
...
function InboxRoutingCard() {
  ...
  const save = async () => {
    ...
    if (!result.ok) {
      setError(result.error || tr("Could not update Inbox routing."));
      return;
    }
    setError(null);
    setDraft("");
  ...
  const clear = async () => {
    ...
    if (!result.ok) {
      setError(result.error || tr("Could not clear Inbox routing."));
      return;
    }
    setError(null);
    load();
  ...
  return (
    <div className={CARD + " p-4"} data-testid="inbox-mirror-card">
      <div className="font-semibold text-[13.5px] mb-1">{tr("Unattended approvals")}</div>
      <p className="text-[12px] text-muted mb-3">
        {tr("Choose where unattended sessions send approval buttons. Current destination:")}{" "}
        <strong className="text-ink font-medium" title={target || undefined}>
          {known ? `#${known}` : target || tr("Link Inbox only")}
        </strong>
        .
      </p>
      ...
      {missingSlackOwner && (
        <p className="text-[11.5px] text-warnInk mt-2">
          {tr("Choose an approval owner under Connectors ▸ Slack before routing approvals here.")}
        </p>
      )}
...
function DmRouteCard() {
  ...
  return (
    <div className={CARD + " p-4"}>
      <div className="font-semibold text-[13.5px] mb-1">{tr("Direct messages")}</div>
      <p className="text-[12px] text-muted mb-3">
        {tr("Choose which session handles direct messages. Without one, messages stay under Unrouted.")}
      </p>
      <div className="flex items-center gap-2">
        ...
        <select className={"flex-1 " + SELECT} value={dm} onChange={(e) => choose(e.target.value)}>
          <option value="">{tr("No session — keep direct messages unrouted")}</option>
          {real.map((s) => (
  ...
  return (
    <div className={CARD + " mb-4 overflow-hidden"}>
      <div className="px-4 py-3 border-b border-line flex items-center gap-2">
        ...
        <span className="font-semibold text-[13.5px]">{tr("Channel subscriptions")}</span>
        <span className="text-[12px] text-muted">{tr("— sessions listening to inbound channel messages")}</span>
      </div>
      ...
      {subs && subs.length > 0 ? (
        <table className="w-full text-[13px]">
          <thead className="text-[11px] uppercase tracking-[0.04em] text-faint">
            <tr className="text-left">
              <th className="font-medium px-4 py-2">{tr("Session")}</th>
              <th className="font-medium px-4 py-2">{tr("Listens to")}</th>
              <th className="font-medium px-4 py-2">{tr("Inbox routes to")}</th>
                ...
                <td className="px-4 py-2.5">
                  ...
                  {s.collision && (
                    <span
                      ...
                      title={tr("This channel is also the inbox route, so inbound and outbound traffic overlap.")}
                    >
                      {tr("⚠ overlaps")}
                    </span>
                  )}
        ...
        <div className="px-4 py-3 text-[12.5px] text-muted">
          {tr("No channel subscriptions yet. Add one below or ask an agent to watch a channel.")}
        </div>
      )}
      ...
      <div className="border-t border-line px-4 py-3 flex items-center gap-2 flex-wrap">
        ...
        >
          <option value="">{tr("Choose a session…")}</option>
          {real.map((s) => (
            <option key={s.session_id} value={s.session_id}>
              {s.title || s.session_id}
            </option>
  ...
  if (items && items.length === 0)
    return (
      <div className={CARD + " p-4 text-[13px] text-muted"}>
        {tr("Nothing here. There are no unrouted messages or failed background tasks.")}
      </div>
    );
  ...
  return (
    <div className={CARD + " overflow-hidden"}>
      <table className="w-full text-[13px]">
        <thead className="text-[11px] uppercase tracking-[0.04em] text-faint">
          <tr className="text-left">
            <th className="font-medium px-4 py-2">{tr("When")}</th>
            <th className="font-medium px-4 py-2">{tr("Source")}</th>
            <th className="font-medium px-4 py-2">{tr("Reason")}</th>
            <th className="font-medium px-4 py-2">{tr("Message")}</th>
          </tr>
        </thead>
        <tbody>
          {(items ?? []).map((it, i) => (
            <tr className="border-t border-line" key={i}>
              <td className="px-4 py-2.5 text-muted whitespace-nowrap">
                {new Date(it.ts * 1000).toLocaleString(locale)}
              </td>
