...
const CARD = "rounded-lg border border-line bg-panel/60";
const BTN_ACCENT =
...
type Source = "all" | "link" | "team";

function sourceOf(p: GalleryPersona): Exclude<Source, "all"> {
  return p.publisher === "Smallink" || p.publisher === "Link" ? "link" : "team";
}
...
export function GalleryModal({
  ...
  const openDetail = async (slug: string) => {
    ...
    setDetail(d ?? { ok: false, error: tr("Could not load details") });
  };
  ...
  const install = async (slug: string) => {
    ...
    setBusy(false);
    if (!r.ok) {
      setMsg(r.error || tr("Install failed"));
      return;
    }
    ...
    <div data-testid="gallery-cards">
      ...
      {unavailable && cloud?.signed_in && (
        <div className="text-[12.5px] text-muted">
          {tr("The agent gallery is unavailable right now. Try again in a moment.")}
        </div>
      )}
        ...
        <>
          ...
          <div className="flex gap-3 overflow-x-auto hairline-scroll pb-2 mb-5" data-testid="gallery-featured">
            {featured.map((p) => (
              <div
                key={p.slug}
                className="w-[240px] shrink-0 rounded-lg border border-line bg-panel/60 overflow-hidden cursor-pointer hover:border-lineStrong"
                onClick={() => openDetail(p.slug)}
              >
                <PersonaHero slug={p.slug} height={88} />
                <div className="p-3">
                  <div className="text-[13px] font-semibold">{p.name}</div>
                  <div className="text-[12px] text-muted leading-snug mt-0.5 mb-2">
                    {tr(p.tagline)}
                  </div>
      ...
      <div className="space-y-2">
          ...
          return (
            ...
            >
              <div className="min-w-0 flex-1">
                ...
                <div className="text-[12.5px] text-muted mb-1.5">{tr(p.tagline)}</div>
              ...
              <div className="shrink-0 flex items-center">
                {isInstalled ? (
                  <span className="text-[12px] text-muted">{tr("Installed")}</span>
                ) : (
        ...
        {visible.length === 0 && !unavailable && (
          <div className="text-[12.5px] text-muted py-4">
            {source === "team"
              ? tr("Nothing has been shared with your team yet.")
              : q
              ? tr("No agents match your search.")
              : tr("No agents have been published yet.")}
          </div>
        )}
      </div>
      ...
      {source !== "team" && teamCount === 0 && (
        <div className="mt-5 pt-3 border-t border-line text-[12px] text-faint" data-testid="gallery-team-teaser">
          {tr(
            "Nothing has been shared by your team yet. Publishing agents to teammates is coming soon.",
          )}
        </div>
      )}
    </div>
  ...
  const detailView = detailSlug && (
    <div data-testid="gallery-detail">
      ...
      {!detail ? (
        <div className="text-[12.5px] text-muted">{tr("Loading…")}</div>
      ) : !detail.ok || !card ? (
        <div className="text-[12.5px] text-danger">
          {detail.error || tr("Could not load details")}
        </div>
      ) : (
        <div className="space-y-4">
          <div className="flex items-start gap-4">
            <div className="min-w-0 flex-1">
              ...
              <div className="text-[13px] text-muted">{tr(card.tagline)}</div>
            ...
            <div className="shrink-0">
              {installed.has(detailSlug) ? (
                <span className="text-[12.5px] text-muted">{tr("Installed")}</span>
              ) : (
          ...
          {justInstalled && (
            <div className="rounded-lg border border-okLine bg-okSoft px-3.5 py-2.5 flex items-center gap-3">
              <span className="flex-1 text-[12.5px] text-ok">
                {tr(
                  "Installed. The agent is disabled until you review and enable it in Settings.",
                )}
              </span>
          ...
          <PersonaHero slug={detailSlug} height={128} className="rounded-lg" />
          ...
          {caps && (
            <div className={CARD + " p-4"} data-testid="gallery-capabilities">
              <div className="text-[13px] font-semibold mb-2">
                {tr("Verified capabilities")}
              </div>
              <div className="text-[12px] text-faint mb-3">
                {tr(
                  "Link reads the agent manifest itself, so these capabilities match the permissions shown during installation. No executable code is installed.",
                )}
              </div>
              <div className="space-y-2 text-[12.5px]">
                <div>
                  <span className="text-muted">{tr("Tools")}: </span>
                  {caps.tools.join(", ") || tr("none")}
                  {caps.risk.length > 0 && (
                    <span className="text-faint">
                      {" "}
                      · {tr("risk")}: {caps.risk.join(", ")}
                    </span>
                  )}
                </div>
                <div>
                  <span className="text-muted">{tr("Permissions")}: </span>
                  {tr(caps.recommended_mode)} {tr("mode")}
                  {caps.messaging ? ` · ${tr("can use messaging")}` : ""}
                </div>
                {(detail.recommends?.filter((r) => r.kind !== "mcp").length ?? 0) > 0 && (
                  <div>
                    <div className="text-muted mb-1.5">
                      {tr("Works with these connections:")}
                    </div>
                      ...
                      {detail.recommends!
                        ...
                        .map((r) => (
                          <div key={r.kind + r.ref} className="flex items-baseline gap-2">
                            ...
                            <span className="text-[12px] text-faint">{tr(r.reason)}</span>
                          </div>
                        ))}
                    </div>
                    <div className="text-[11.5px] text-faint mt-2">
                      {tr(
                        "You connect these services yourself. Installing an agent grants no connection access until you do.",
                      )}
                    </div>
                  </div>
                )}
  ...
  return (
    <div className="fixed inset-0 z-50" data-testid="gallery-modal">
      ...
      <div className="absolute left-1/2 top-[6vh] -translate-x-1/2 w-[720px] max-w-[94vw] max-h-[88vh] rounded-lg border border-line bg-panel shadow-2xl overflow-hidden flex flex-col">
        <div className="px-5 pt-4 pb-3 border-b border-line flex items-center gap-3 shrink-0">
          <div className="min-w-0 flex-1">
            <div className="text-[15px] font-semibold">{tr("Agent gallery")}</div>
            <div className="text-[12px] text-muted">
              {tr("Curated agents · disabled until you review and enable them")}
            </div>
          </div>
          ...
          <button
            ...
            onClick={onClose}
            aria-label={tr("Close agent gallery")}
          ...
          {loading ? (
            ...
            <div className={CARD + " p-5"} data-testid="gallery-unavailable">
              <div className="font-semibold text-[14px] mb-1">
                {tr("Agent gallery is not configured")}
              </div>
              <div className="text-[12.5px] text-muted leading-relaxed">
                {tr(
                  "This Link build runs locally. Install an agent from a folder or Git URL in Settings.",
                )}
              </div>
            </div>
            ...
            <div className={CARD + " p-5 flex items-center gap-4"} data-testid="gallery-signin">
              <div className="min-w-0 flex-1">
                <div className="font-semibold text-[14px] mb-1">
                  {tr("Sign in to browse the agent gallery")}
                </div>
                <div className="text-[12.5px] text-muted leading-relaxed">
                  {tr(
                    "The gallery contains curated agents from Link Cloud and requires a free cloud sign-in. Local folders and Git URLs can always be installed without an account.",
                  )}
                </div>
              </div>
              <button className={BTN_ACCENT} onClick={signIn} disabled={signingIn}>
                {signingIn ? tr("Check your browser…") : tr("Sign in")}
              </button>
            </div>
          ) : detailSlug ? (
            detailView
