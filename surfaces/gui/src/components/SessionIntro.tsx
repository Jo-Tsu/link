...
// Empty-state for a fresh Link session (§27): a greeting, exactly three concrete template
...
export function SessionIntro({
  ...
  const pickFolder = () => {
    ...
    if (shared.length > 0) onPrefill(tr(FOLDER_PROMPT));
  ...
  return (
    <div className="intro">
      <h1 className="greeting">
        <span className="mark"><Icon name="logo" size={19} /></span> {tr("What should we produce?")}
      </h1>
      <p className="intro-lede">
        {tr("Pick a task to start — I'll do the work and save the result. Or just type what you need below.")}
      </p>

      <div className="intro-tasks">
        <button className="task-card" data-testid="intro-task-folder" onClick={pickFolder}>
          <span className="task-card-body">
            <span className="task-card-title">{tr("Analyze the files in a directory")}</span>
            <span className="task-card-sub">{tr("I'll read them and summarize what matters")}</span>
          </span>
          <span className="task-card-act">{tr("Pick a folder →")}</span>
        </button>
        {addingFolder && (
          <div className="intro-addfolder">
            <AddFolderForm
              ...
              onAdd={async (path, writable) => {
                ...
                if (ok !== false) onPrefill(tr(FOLDER_PROMPT));
                return ok;
              }}
        ...
        >
          <span className="task-card-body">
            <span className="task-card-title">{tr("Create a report from my HubSpot leads")}</span>
            <span className="task-card-sub">
              {dot("hubspot", hubspotReady)}
              {tr("Sources, stages, and who needs follow-up")}
            </span>
          </span>
        ...
        >
          <span className="task-card-body">
            <span className="task-card-title">{tr("Automate a weekly GitHub progress report to Slack")}</span>
            <span className="task-card-sub">
              {dot("github", live.has("github"))}
              {dot("slack", live.has("slack"))}
              {tr("Repo activity, summarized and posted every Friday")}
            </span>
          </span>
