...
export function Composer(props: Props) {
  ...
  useEffect(() => {
    ...
    refresh();
    window.addEventListener("link:voice-input-changed", refresh);
    return () => window.removeEventListener("link:voice-input-changed", refresh);
  }, []);
  ...
  const addFiles = async (files: FileList | File[]) => {
    ...
    for (const file of list) {
      if (isPdfFile(file) && file.size > maxMb * 1024 * 1024) {
        showAttachNotice(
          tr("{name} skipped — {size} MB is over your {limit} MB limit (Settings → Token savings)", {
            name: file.name,
    ...
    for (const a of read) {
      if (a.kind === "pdf" && a.data_url) {
        ...
        if (info?.ok && (info.pages ?? 0) > maxPages) {
          showAttachNotice(
            tr("{name} skipped — {pages} pages is over your {limit}-page limit (Settings → Token savings)", {
              name: a.name,
              pages: info.pages ?? 0,
              limit: maxPages,
            }),
        ...
        if (info && !info.ok) {
          showAttachNotice(
            tr("{name} skipped — {error}", {
              name: a.name,
              error: info.error || tr("could not read PDF"),
            }),
          );
          continue;
        }
  ...
  const toggleDictation = async () => {
    ...
    try {
      ...
      if (!status) throw new Error(tr("Voice dictation is unavailable."));
      ...
      if (!recording?.recording) throw new Error(tr("Could not start the microphone."));
      setDictation(recording);
    } catch (error) {
      setDictationError(error instanceof Error ? error.message : tr("Voice dictation is unavailable."));
  ...
  const dictationBusyLabel =
    dictationBusy === "transcribing"
      ? tr("Transcribing…")
      : dictationBusy === "starting"
        ? tr("Starting microphone…")
        : "";
  ...
  return (
    <div className="composer-wrap px-6 pb-5 pt-4">
      ...
      >
        {props.projectName && (
          <div className="flex items-center gap-1.5 px-3.5 pt-2.5 pb-0">
            <span className="inline-flex items-center gap-1 rounded-md bg-faint/20 px-2 py-0.5 text-[12px] text-muted font-medium">
              <span className="text-[13px] leading-none">{props.projectIcon || "\u{1F4C1}"}</span>
              {props.projectName}
            </span>
          </div>
        )}
        <textarea
          ...
          placeholder={props.placeholder || tr("Ask the agent…  (drop or paste files)")}
          value={text}
        ...
        <div className="px-2.5 pb-2.5 pt-1 flex items-center gap-1.5">
            ...
            {attachMenuOpen && (
              <>
                ...
                <div className="absolute z-40 bottom-full mb-1 left-0 min-w-[180px] rounded-xl border border-line bg-panel shadow-2xl py-1.5">
                  {attachItem("image", tr("Photo or image"), () => pickFiles("image/*"))}
          ...
          {dictationBusy === "transcribing" && (
            <span className="text-[11.5px] text-accent">{tr("Transcribing…")}</span>
          )}
          ...
          {!dictation?.recording && (needsModel ? (
            <button
              ...
              aria-label={tr("No model connected — connect a model")}
            >
              <span className="pill-label">{tr("No model")}</span>
              <Icon name="plug" size={12} className="model-warn-ico" />
            </button>
          ) : modelsLoaded ? (
            ...
            <button
              ...
              title={tr("Fetching the model list from the server")}
            >
              <span className="pill-label">{tr("Loading models…")}</span>
            </button>
          ))}
          ...
          {isTauri() && (
            <button
              className={
                iconBtn +
                (dictation?.recording ? " bg-dangerSoft text-danger hover:bg-dangerSoft/80" : "") +
              ...
              title={
                dictationBusyLabel ||
                (dictation?.recording
                  ? tr("Stop recording and transcribe")
                  : voiceReady
                    ? tr("Start local voice dictation")
                    : tr("Configure Voice Input in Settings"))
              }
              aria-label={tr(dictation?.recording ? "Stop dictation" : voiceReady ? "Start dictation" : "Configure Voice Input in Settings")}
          ...
          {props.running ? (
            <button className="btn danger" onClick={props.onInterrupt}>
              <Icon name="stop" size={14} /> {tr("Stop")}
            </button>
          ) : (
            <button
              className={
      ...
      <span className="sr-only" role="status" aria-live="polite">
        {dictation?.recording ? tr("Listening, {time}", { time: recordingTime }) : dictationBusyLabel}
      </span>
    </div>
  );
}
  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        aria-label={tr("Mode")}
        title={
          tr("Mode: {mode}", { mode: tr(current?.label || mode) }) +
          (unattended ? ` · ${tr("approvals go to the Inbox")}` : "")
        }
      >
        {tr(current?.label || mode)}
          ...
          >
            {PERMISSION_OPTIONS.map((o) => (
              ...
              >
                ...
                <span className="text-[11px] text-faint leading-snug">{tr(o.description || "")}</span>
              </button>
            ))}
            {onUnattendedChange && (
              <>
                ...
                <div className="flex items-center gap-2 px-2.5 py-1.5">
                  <span className="flex-1 min-w-0">
                    <span className="block text-[13px] text-ink">{tr("Send approvals to Inbox")}</span>
                    <span className="block text-[11px] text-faint leading-snug">
                      {tr("Approvals & questions go to the Inbox; the agent keeps working.")}
                    </span>
                  </span>
                  <Toggle
                    checked={!!unattended}
                    onChange={onUnattendedChange}
                    title={tr("Send approvals to the Inbox")}
                  />
                </div>
              </>
            )}
...
function AttachChip({ a, onRemove }: { a: Attachment; onRemove: () => void }) {
  ...
  return (
    <div className={"attach-chip" + (a.kind === "image" ? " img" : "")}>
      ...
      <button className="attach-x" onClick={onRemove} title={tr("Remove")} aria-label={tr("Remove")}>
        ✕
      </button>
    </div>
  );
