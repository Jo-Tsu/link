...
import { isConnectorVisible } from "./connectors/visibility";
...
interface QuickTemplate {
  ...
  time: string;
  instructions: (
    ctx: { repo: string; channel: string; deliver: "app" | "slack" },
    tr: Translator,
  ) => string;
}
...
const TEMPLATES: QuickTemplate[] = [
  {
    ...
    day: "mon",
    time: "09:00",
    instructions: ({ repo, channel }, tr) =>
      tr("Summarize activity since the last digest in the GitHub repository {repo}: merged pull requests, notable commits, and anything needing attention. Post the digest to the Slack channel {channel} using send_message.", {
        repo: repo || tr("the connected repository"),
        channel,
      }),
  },
  {
    ...
    day: "mon",
    time: "09:00",
    instructions: ({ channel }, tr) =>
      tr("Review HubSpot activity since the last digest: deals that changed stage, deals going quiet, and deals past their close date. Post a short pipeline digest to the Slack channel {channel} using send_message.", {
        channel,
      }),
  },
  {
    ...
    day: "daily",
    time: "08:00",
    instructions: ({ deliver }, tr) =>
      tr(
        deliver === "app"
          ? "Prepare a short morning brief: today's calendar events and gaps, plus email that arrived since yesterday evening. Save it as the session deliverable."
          : "Prepare a short morning brief: today's calendar events and gaps, plus email that arrived since yesterday evening. Send it to me as a Slack DM.",
      ),
  },
  {
    key: "news",
    ...
    day: "daily",
    time: "08:00",
    instructions: (_ctx, tr) =>
      tr("Search the web for the most important technology and world news from the last 24 hours and write a concise 5-bullet briefing, saved as a markdown file."),
  },
  {
    key: "inboxdigest",
    title: "Inbox digest",
    ...
    day: "weekdays",
    time: "09:00",
    instructions: (_ctx, tr) => tr("Summarize my unread email into one short digest note."),
  },
  {
    key: "cleanup",
    ...
    day: "fri",
    time: "17:30",
    instructions: (_ctx, tr) => tr("Sort my recent Downloads into tidy folders by file type."),
  },
];
...
export function AutomationQuickstart({
  ...
  const visibleTemplates = TEMPLATES.filter((template) =>
    template.conns.every((connector) => isConnectorVisible(connector.name)),
  );
  const [pickedKey, setPickedKey] = useState<string | null>(null);
  const picked = visibleTemplates.find((t) => t.key === pickedKey) || null;
  ...
  const create = () => {
    ...
    onCreate({
      ...
      instructions: picked.instructions({ repo, channel, deliver }, tr),
  ...
  const missingConnectors = picked?.conns
    .filter((c) => !connState(c.name)?.connected)
    .map((c) => connState(c.name)?.title || c.name) || [];
  const gateHint = !allConnected
    ? tr("Connect {names} to continue", {
        names: missingConnectors.join(language === "zh-CN" ? "、" : " and "),
      })
    : picked?.needsChannel && !channel
      ? tr("Pick a channel to post to first")
      : "";
  ...
  return (
    <div className="mb-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 auto-rows-fr gap-3">
        {visibleTemplates.map((t) => (
          <button
            ...
            className={
              "h-full text-left rounded-lg border bg-panel p-4 flex flex-col gap-1.5 " +
              (pickedKey === t.key
                ? "border-accent ring-2 ring-accentSoft"
                : "border-line hover:border-lineStrong")
            }
            onClick={() => pick(t)}
            aria-pressed={pickedKey === t.key}
          >
            ...
            <span className="flex items-center gap-1.5 mt-1">
              {t.conns.map((c) => {
                ...
                return (
                  <span
                    key={c.name}
                    title={tr(on ? "{name} — connected" : "{name} — not connected yet", {
              ...
              <span className="text-[11px] text-faint ml-0.5">
                {t.conns.length === 0
                  ? tr("No connections needed · {cadence}", { cadence: tr(t.cadence) })
                  : tr(t.cadence)}
              </span>
            </span>
          </button>
      ...
      {picked && (
        <div
          ref={cfgRef}
          className="mt-3 rounded-lg border border-line bg-panel p-4"
        ...
        >
          ...
          <div className="flex items-baseline gap-2 pb-2.5 mb-1 border-b border-line">
            ...
            <span className="ml-auto text-[12px] text-faint max-sm:hidden">
              {tr(picked.conns.length ? "Connections, delivery & schedule" : "Delivery & schedule")} ·{" "}
              {tr(picked.cadence)}
            </span>
          </div>
              ...
              <div key={name} className="border-b border-line last:border-b-0">
                <div className="flex items-center gap-3 py-2.5">
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="block text-[11.5px] text-faint">{tr(why)}</span>
                  </span>
                  {c?.connected ? (
                    <span className="text-[12.5px] text-ok">✓ {tr("Connected")}</span>
                  ) : flow ? (
                    <span className="inline-flex items-center gap-2 text-[12px] text-muted">
                      <Spinner />
                      {flow.phase === "opening"
                        ? tr("Opening browser…")
                        : tr("Waiting for {name}…", { name: c?.title || name })}
                    </span>
                  ) : (
                    <button
                  ...
                  >
                    ...
                    <span className="flex-1 min-w-0">
                      <b className="text-ink font-medium">
                        {tr("Finish connecting {name} in your browser.", { name: c?.title || name })}
                      </b>{" "}
                      {tr("Approve it there, then come back — this page updates by itself.")}
                    </span>
                    <button
          ...
          {pendingConn && !cloud?.signed_in && cloud?.available === false && (
            <div
              className="bg-paper rounded-xl px-4 py-3 mt-3 text-[12.5px] text-muted"
              data-testid="ob-local-connector-pane"
            >
                ...
                {signinPhase ? (
                  <>
                    <span className="inline-flex items-center gap-2 text-[12px]">
                      <Spinner />
                      {tr(signinPhase === "opening" ? "Opening browser…" : "Waiting for sign-in…")}
                    </span>
                    {signinPhase === "waiting" && (
                      <span className="text-[11.5px] text-faint">
                        {tr("Finish signing in in your browser — this page updates by itself.")}{" "}
                        <button
          ...
          {allConnected && (
            <div className={picked.conns.length ? "bg-paper rounded-xl px-4 py-3.5 mt-3" : ""} data-testid="ob-recipe">
              {picked.needsRepo && (
                <>
                  <label className={label}>{tr("Repository")}</label>
                  <input
                    className={input}
              ...
              {picked.needsChannel && (
                <>
                  <label className={label}>{tr("Post to channel")}</label>
                  ...
                  <p className="text-[11px] text-warnInk mt-1">
                    {tr("The bot must be a member of the channel — invite @Link in Slack if it isn't.")}
                  </p>
                </>
              )}
              <label className={label}>{tr("When")}</label>
              <div className="flex gap-2">
                <div className="flex-1 min-w-0">
                  <SelectMenu
                    ariaLabel={tr("Day")}
                    value={day}
                    options={Object.entries(DAYS).map(([k, v]) => ({ value: k, label: tr(v.label) }))}
                    onChange={setDay}
                  />
                </div>
                <input
              ...
              {picked.deliver && (
                <>
                  <label className={label}>{tr("Deliver to")}</label>
                  <SelectMenu
                    ariaLabel={tr("Deliver to")}
                    value={deliver}
                    options={[
                      { value: "app", label: tr("In the app") },
                      { value: "slack", label: tr("Slack DM (connect Slack later)") },
                    ]}
              ...
              {picked.consent ? (
                <label className="flex items-start gap-2.5 mt-3.5 text-[12.5px] text-muted select-none">
                  ...
                  />
                  <span>
                    {tr("Allow this automation to post its digest to {channel} without asking each time. Anything else still asks first.", {
                      channel:
                        (channelLabel || tr("the channel")) +
                ...
                <p className="text-[12.5px] text-muted mt-3">
                  {tr("This automation only reads on schedule — reading never needs approval.")}
                </p>
              ) : null}
            </div>
          )}
