...
export function FolderGate({ onChoose, onCancel, create }: Props) {
  ...
  const open = async (p: string, doCreate = false) => {
    ...
    else setError(res.error || tr("could not open that folder"));
  };
  ...
  return (
    <div className="gate-overlay" role="dialog" aria-modal="true" aria-labelledby="folder-gate-title">
      <div className="gate">
        <div className="gate-mark"><Icon name="logo" size={24} /></div>
        <h2 id="folder-gate-title">{tr(create ? "New project" : "Choose a project folder")}</h2>
        <p className="gate-sub">
          {tr(create
            ? "Pick a folder or enter a path. If the path doesn't exist, it will be created."
            : "This agent needs a workspace to read, edit, and run in.")}
        </p>

        <div className="gate-input">
          <input
            placeholder={tr("/path/to/your/project")}
            value={path}
            onChange={(e) => setPath(e.target.value)}
            onKeyDown={(e) => e.key === "Enter" && open(path, create)}
            autoFocus
          />
          <button className="btn" onClick={browse} title={tr("Pick a folder")}>
            {tr("Browse…")}
          </button>
          <button className="btn primary" onClick={() => open(path, create)} disabled={!path.trim()}>
            {tr(create ? "Create" : "Open")}
          </button>
        </div>
        ...
        {recents.length > 0 && (
          <>
            <div className="gate-label">{tr("Recent")}</div>
            <div className="gate-recents">
              {recents.map((w) => (
                <div className="gate-recent" key={w.path} onClick={() => open(w.path)} title={w.path}>
                  <span className="folder"><Icon name="folder" size={14} /> {w.name}</span>
