...
export function InboxView({
  ...
  const slackAvailable = isConnectorVisible("slack");
  ...
  const sessionChip = (it: InboxItem) => {
    ...
    const cls = `ico-${p?.icon || "link"}`;
    return (
      <button
        className="inbox-session-chip"
        title={exists ? tr("Open “{name}”", { name: label }) : tr("Session unavailable")}
        disabled={!exists}
        onClick={() =>
          exists && onOpenSession(it.session_id, it.session_workspace || "", it.session_agent || "link")
        }
      >
    ...
    <main className="flex-1 min-w-0 flex bg-paper">
      <div className="flex-1 min-w-0 overflow-y-auto hairline-scroll">
        <div className="max-w-4xl mx-auto px-7 py-6">
          <PanelHead
            title={tr("Inbox")}
            sub={tr("Review approvals, questions, and notifications from every agent, including unattended sessions.")}
          />
          ...
          {tab === "configure" ? (
            ...
            <>
              <div className="text-[12px] text-faint -mt-1 mb-4" data-testid="inbox-routing">
                {routing ? (
                  <span>
                    {tr("Also delivered to")}{" "}
                    <span className="text-muted" title={routing}>
                      {routingLabel}
                    </span>{" "}
                    {tr("— replies there resolve items here.")}{" "}
                  </span>
                ) : slackConnected ? (
                  <span>{tr("Delivered here only.")} </span>
                ) : slackAvailable ? (
                  <span>
                    {tr("Delivered here only. Connect Slack from the Connectors page to also receive these in a channel.")}{" "}
                  </span>
                ) : (
                  <span>{tr("Delivered here only.")} </span>
                )}
              ...
              {visible.length === 0 ? (
                <div className="manage-empty">
                  {tr(items.length === 0 ? "Nothing pending." : "Nothing pending for this filter.")}
                </div>
              ) : null}
