...
export function scopeNote(
  ...
  if (EXTERNAL.has(name)) {
    ...
    return {
      text: tr("leaves this Mac → {target}", {
        ...
        <button className="approval-prev-more" onClick={() => setAll((v) => !v)}>
          {all
            ...
            : lines.length > PREVIEW_LINES
              ? tr("show all {count} lines", { count: lines.length })
              : tr("show the full message")}
        </button>
      )}
    </div>
  );
  ...
  return (
    <div className="approval-btns">
      ...
      {offerStanding && (
        <button
          className="btn"
          title={tr("Always allow {tool} → {target} for “{task}” — revoke any time on its Automations page", {
            tool: item.name,
            target: item.standingTarget || "",
            task: runTask?.title || tr("this automation"),
          })}
      ...
      {!connector && !offerStanding && item.name !== "run_shell" && (
        <button
          className="btn"
          title={tr("Always allow {tool} for this session", {
            tool: tr(TOOL_VERBS[item.name] || item.name).toLocaleLowerCase(),
          })}
...
export function ApprovalCard({
  ...
  const title = humanizeApprovalTitle(item.name, item.args, tr);
  const scope = scopeNote(item.name, item.args, item.category, tr);
    ...
    return (
      <div className={"approval approval-row" + dock} data-testid="approval-row">
        <div className="approval-row-line">
          ...
          <span className="spacer" />
          <Buttons item={item} onApprove={onApprove} runTask={runTask} primaryLabel={tr("Allow")} />
        </div>
    ...
    <div className={"approval" + (scope.external ? " approval-external" : "") + dock}>
      <div className="approval-top">
        <div className="approval-heading">
          <span className="approval-ico" title={tr("Tool: {name}", { name: item.name })}>
      ...
      {item.name === "send_file" && (
        <>
          <span className="approval-filechip">
            ...
            {String(item.args?.path ?? "").split("/").pop() || tr("file")}
            {item.args?.as_screenshot ? ` · ${tr("as a PNG screenshot")}` : ""}
          </span>
          {item.args?.comment && (
            <MessagePreview text={String(item.args.comment)} label={tr("With the message")} />
          )}
        </>
      )}
          ...
          {grants.map((g, i) => (
            <div className="approval-grant" key={i} data-access={g.access}>
              ...
              <span className="grant-line">
                {tr(TOOL_VERBS[g.tool] || g.tool)} <code className="approval-tool">{g.target}</code>
                <span className="grant-note">
                  {" — "}
                  {tr(g.access === "write" ? "always allowed once you approve" : "read-only")}
                </span>
              </span>
            </div>
          ))}
      ...
      {item.resolved ? (
        <div className="resolved">
          {tr("Approved: {decision}", { decision: tr(item.resolved.replace(/_/g, " ")) })}
        </div>
      ) : (
        <Buttons item={item} onApprove={onApprove} runTask={runTask} primaryLabel={tr("Allow once")} />
      )}
    </div>
  );
}
