...
export function InboxItemCard({
  ...
  return (
    <div
      className={
        compact
          ? "max-w-3xl mx-auto mb-2.5 rounded-lg border border-lineStrong bg-panel px-4 py-3.5 shadow-[0_8px_24px_rgba(0,0,0,0.08)]"
          : "mb-2.5 rounded-lg border border-line bg-panel px-3.5 py-3"
      }
    >
      ...
      {item.kind === "approval" && item.data?.tool ? (
        <div className="flex items-center justify-between gap-3">
          <TitleText line={humanizeApprovalTitle(item.data.tool, item.data.arguments, tr)} />
          {(() => {
            const s = scopeNote(item.data.tool, item.data.arguments, undefined, tr);
            return (
      ...
      {item.kind === "approval" ? (
        <div className="flex items-center gap-2 mt-2.5 flex-wrap">
          ...
          >
            {tr(item.data?.tool ? "Allow once" : "Approve")}
          </button>
          ...
          {item.data?.task_id && item.data?.standing_target && (
            <button
              ...
              title={tr("Always allow against {target} for “{task}”. Revoke it anytime from the Automations page.", {
                target: item.data.standing_target,
                task: item.data.task_title || tr("this automation"),
              })}
        ...
        <>
          ...
          {multi && options.length > 0 && (
            <div className="mt-2.5">
              ...
              >
                {tr("Send")}{selected.length ? ` (${selected.length})` : ""}
              </button>
            </div>
          )}
          {(allowText || options.length === 0) &&
            textRow(tr(options.length ? "Or type your own answer…" : "Your answer…"))}
        </>
      ) : item.kind === "directory" ? (
        <div className="flex items-center gap-2 mt-2.5">
          <button
            ...
            title={item.data?.path || tr("No folder was suggested")}
            onClick={() =>
              onResolve(
                item.id,
          ...
          >
            {tr(item.data?.path ? "Grant" : "Grant (no folder)")}
          </button>
