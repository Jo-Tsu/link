...
function relativeTime(
  ...
  if (diff < 0) return tr("just now");
  if (diff < 45_000) return tr("just now");
  const mins = Math.round(diff / 60_000);
  if (mins < 60) return tr("{count}m ago", { count: mins });
  const hrs = Math.round(diff / 3_600_000);
  if (hrs < 24) return tr("{count}h ago", { count: hrs });
  const days = Math.round(diff / 86_400_000);
  if (days < 7) return tr("{count}d ago", { count: days });
  return new Date(then).toLocaleDateString(locale);
}

/** Absolute clock time (for the time element's title), e.g. "2:14 PM". */
function clockTime(tsSeconds: number, locale: string): string {
  if (!tsSeconds || !isFinite(tsSeconds)) return "";
  return new Date(tsSeconds * 1000).toLocaleTimeString(locale, { hour: "numeric", minute: "2-digit" });
}
      ...
      >
        ...
        {showIds ? (
          ...
          <>
            ...
            <span className="text-[11px] text-faint ml-0.5">{tr("via {label}", { label: entry.label })}</span>
          </>
        )}
        <time className="ml-auto text-[11px] text-faint whitespace-nowrap" title={clockTime(source.ts, locale)}>
          {relativeTime(source.ts, tr, locale)}
        </time>
      </header>
