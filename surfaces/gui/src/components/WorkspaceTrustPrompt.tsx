...
export function WorkspaceTrustPrompt({
  ...
  const trust = async () => {
    ...
    setSaving(false);
    if (!result?.ok) {
      setError(result?.error || tr("Could not save workspace trust."));
      return;
    }
    onClose();
  };
  ...
  return (
    <div className="gate-overlay" role="dialog" aria-modal="true" aria-labelledby="workspace-trust-title">
      <div className="gate max-w-[560px]">
        ...
        <h2 id="workspace-trust-title">{tr("Trust this workspace’s commands?")}</h2>
        <p className="gate-sub">
          {tr("This project asks Smallink to run the commands below without individual approval. Trust applies to future configuration changes at this exact folder until you revoke it in Settings.")}
        </p>
        ...
        <div className="gate-foot justify-end gap-2">
          ...
          <button className="btn primary" onClick={() => void trust()} disabled={saving}>
            {tr(saving ? "Saving…" : "Trust workspace")}
          </button>
        </div>
      </div>
    </div>
