  ...
  return (
    <div data-testid="gmail-detail">
      ...
      <div className={FOOT + " mt-2"}>
        {tr("Filters run on this computer before agents see results. Activity shows hidden counts, never hidden content.")}
      </div>
    </div>
  );
}
...
function FiltersGroup({ c, onChanged }: Pick<DetailProps, "c" | "onChanged">) {
  ...
  return (
    <>
      <div className={GRP_H}>{tr("Never show agents")}</div>
      <div className={GRP} data-testid="gmail-filters">
        <ChipListRow
          label={tr("Senders")}
          testid="gmail-filter-senders"
          placeholder={tr("name@example.com or @domain.com")}
        ...
        <ChipListRow
          ...
          placeholder={tr("Label name, e.g. Personal")}
      ...
      <div className={FOOT}>
        {tr("Matching email is removed before agents read results, leaving no trace they can inspect.")}
      </div>
    </>
  );
}
