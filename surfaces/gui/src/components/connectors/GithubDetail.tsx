...
function relayHealth(
  ...
  if (!gh) return { dot: "bg-ok", text: tr("Live · managed relay") };
  if (!gh.signed_in)
    ...
    <div data-testid="github-installations">
              ...
              <>
                <span
                  className={
                    ...
                    (relay ? relayHealth(status, tr).dot : "bg-ok")
                  }
                />
                <span data-testid="github-mode-badge">
                  {relay
                    ? relayHealth(status, tr).text
                    : tr("Connected · personal access token")}
                </span>
              </>
            ) : (
      ...
      {!c.connected && (
        <div className={GRP}>
          <div className={ROW + " text-[12.5px] text-muted"}>
            {tr(
              "Install the @link-agent App for each account or organization, choose repositories on GitHub, and manage a separate allow-list for every installation.",
            )}
            {cloud?.signed_in
              ? ""
              : ` ${tr("One-click setup needs cloud sign-in; a personal access token works without it.")}`}
          </div>
        </div>
      )}
      ...
      {c.connected && !relay && (
        <div className={GRP} data-testid="github-manual-card">
          <div className={ROW + " text-[12.5px] text-muted"}>
            {tr(
              "Personal access token · tools only. Install the GitHub App to send @mentions and agent labels to this computer.",
            )}
          </div>
        </div>
      )}
      ...
      {relay && listening.length > 0 && (
        <>
          <div className={GRP_H}>{tr("Listening")}</div>
          <div className={GRP}>
      ...
      {c.connected && relay && (
        <div className={FOOT + " mt-2"}>
          {tr(
            "Triggers: @link-agent mentions and the “link-agent” label. The agent replies as link-agent[bot].",
          )}
        </div>
      )}
      ...
      <div className={GRP_H + " flex items-center gap-2"}>
        <span>
          ...
          <span className="font-normal text-faint" title={`installation ${inst.installation_id}`}>
            · {inst.repo_selection === "all" ? tr("all repositories") : tr("selected repositories")}
          </span>
        </span>
        {!tokenOk && (
          <span className={TAG_WARN} data-testid={`token-warn-${inst.installation_id}`}>
            {tr("Installation revoked. Reinstall it.")}
          </span>
        )}
      </div>
      <div className={GRP}>
        {empty ? (
          <div className={ROW}>
            <span className="min-w-0 flex-1 text-[12.5px] text-muted">
              {tr("No one is allowed yet. @link-agent mentions will appear here for approval.")}
            </span>
...
function DisconnectBtn({ id, busy, onClick }: { id: string; busy: boolean; onClick: () => void }) {
  ...
  return (
    <button
      ...
      title={tr(
        "Stop relaying this installation to this computer. The App stays installed on GitHub.",
      )}
      onClick={onClick}
      disabled={busy}
    >
      {busy ? tr("Disconnecting…") : tr("Disconnect installation")}
    </button>
  );
}
      ...
      <span className="min-w-0 flex-1 flex flex-wrap items-center gap-1.5">
        {allowed.length === 0 && (
          <span className="text-[12px] text-faint">
            {tr("No one yet. Approve a waiting sender below.")}
          </span>
        )}
        {allowed.map((login) => (
          <span
  ...
  return (
    <div className={ROW + " bg-warnSoft/25"} data-testid={`waiting-${m.id}`}>
      ...
      <button
        ...
        title={tr("Allow the sender and deliver this mention now")}
      ...
      <button
        ...
        title={tr("Allow the sender; discard this mention")}
        onClick={() => act("allow")}
      >
        {tr("Allow")}
      </button>
