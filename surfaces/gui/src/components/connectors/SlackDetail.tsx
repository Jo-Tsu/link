...
function relayHealth(
  ...
  if (!slack) return { dot: "bg-ok", text: tr("Live · managed relay") };
  if (!slack.signed_in)
    return { dot: "bg-warnInk", text: tr("Sign-in needed. Relaying is paused.") };
  if (slack.relay.state === "offline")
    return { dot: "bg-faint/60", text: tr("Offline. The relay cannot be reached.") };
  if (slack.relay.state === "reconnecting")
    return { dot: "bg-warnInk", text: tr("Reconnecting to the relay…") };
  return { dot: "bg-ok", text: tr("Live · managed relay") };
}
    ...
    <div data-testid="slack-workspaces">
              ...
              <>
                <span
                  className={
                    ...
                    (relay ? relayHealth(slack, tr).dot : "bg-ok")
                  }
                />
                <span data-testid="slack-mode-badge">
                  {relay
                    ? relayHealth(slack, tr).text
                    : tr("Connected · Socket Mode (manual tokens)")}
                </span>
              </>
            ) : (
      ...
      {!c.connected && (
        <div className={GRP}>
          <div className={ROW + " text-[12.5px] text-muted"}>
            {tr(
              "Install one @link app per workspace. Each workspace keeps its own allow-list.",
            )}
            {cloud?.signed_in
              ? ""
              : ` ${tr("One-click setup needs cloud sign-in; manual setup works without it.")}`}
          </div>
        </div>
      )}
      ...
      {c.connected && !relay && (
        <div data-testid="slack-manual-card">
          <div className={GRP_H}>
            {c.account || tr("workspace")}{" "}
            <span className="font-normal text-faint">· {tr("manual tokens")}</span>
          </div>
          <div className={GRP}>
            <PeopleRow
      ...
      {c.connected && (
        <div className={FOOT + " mt-2"}>
          {tr("Names come from Slack automatically. IDs appear on hover.")}
        </div>
      )}
      ...
      <div className={GRP_H + " flex items-center gap-2"}>
        ...
        {!tokenOk && (
          <span className={TAG_WARN} data-testid={`token-warn-${w.team_id}`}>
            {tr("Token revoked. Reinstall the app.")}
          </span>
        )}
      </div>
      <div className={GRP}>
          ...
          <>
            <div className={ROW}>
              <span className="min-w-0 flex-1 text-[12.5px] text-muted flex items-center gap-2 flex-wrap">
                <span>{tr("No one is allowed yet. Bot mentions will appear here for approval.")}</span>
...
function DisconnectBtn({ teamId, busy, onClick }: { teamId: string; busy: boolean; onClick: () => void }) {
  ...
  return (
    <button
      ...
      title={tr(
        "Stop relaying this workspace to this computer. The app stays installed in Slack.",
      )}
      onClick={onClick}
      disabled={busy}
    >
      {busy ? tr("Disconnecting…") : tr("Disconnect workspace")}
    </button>
  );
}
...
function PeopleRow({
  ...
  const label = (u: string) =>
    names?.[u] || (u === installerId ? installerName || tr("You") : u);
  return (
    <div className={ROW}>
      <span className={LABEL}>{tr("People")}</span>
      <span className="min-w-0 flex-1 flex flex-wrap items-center gap-1.5">
        {allowed.length === 0 && (
          <span className="text-[12px] text-faint">
            {tr("No one yet. Pick a name or approve a waiting sender below.")}
          </span>
        )}
        {allowed.map((u) => (
          <span
          ...
          >
            ...
            {label(u)}
            {u === installerId && (
              <span className="text-[10.5px] text-faint">· {tr("you")}</span>
            )}
            {protectedIds?.includes(u) ? (
              <span
                className="text-[10.5px] text-faint"
                title={tr("Remove approval-owner access before removing this person.")}
              >
                · {tr("owner")}
              </span>
            ) : (
              <button className={XBTN} title={tr("Remove")} onClick={() => onRemove(u)}>
                ×
              </button>
            )}
          </span>
...
function PersonPicker({
      ...
      getSlackDirectory(teamId || "default", q)
        .then((r) => {
          if (r.ok) {
            setRows(r.members || []);
            setErr(null);
          } else setErr(r.error || tr("Directory unavailable"));
        })
        .catch(() => setErr(tr("Directory unavailable")));
    }, 200);
  ...
  const pick = async (m: SlackMember) => {
    ...
    if (result?.ok === false) {
      setErr(result.error || tr("Could not add person"));
      return;
    }
    setOpen(false);
    setQ("");
  ...
  return (
    <span className="relative" ref={wrap}>
      <button
        ...
        title={tr("Pick from the workspace directory")}
        onClick={toggle}
      >
        {buttonLabel || `＋ ${tr("Add person")}`}
      </button>
      {open && (
        <div
          className="fixed z-50 w-72 rounded-lg border border-line bg-panel shadow-lg p-1"
        ...
        >
          ...
          <div className="max-h-56 overflow-y-auto py-1">
            {err ? (
              ...
              <div className="px-2 py-1.5 text-[12px] text-faint">{tr("No matches")}</div>
            ) : (
              candidates.map((m) => (
                <button
                  key={m.id}
          ...
          <div className="px-2 pb-1 text-[10.5px] text-faint">
            {tr("From your workspace directory. This data stays on this computer.")}
          </div>
        </div>
      )}
    </span>
...
function ApprovalOwnersRow({
  ...
  const remove = async (userId: string) => {
    ...
    if (!result.ok) {
      setErr(result.error || tr("Could not remove approval owner"));
      return;
    }
    setErr(null);
    onChanged();
  ...
  return (
    <div className={ROW} data-testid="slack-approval-owners">
      <span className={LABEL}>{tr("Approvals")}</span>
      <span className="min-w-0 flex-1 flex flex-wrap items-center gap-1.5">
        {owners.length === 0 && (
          <span className="text-[12px] text-warnInk">
            {tr("Choose at least one owner before routing Inbox approvals to Slack.")}
          </span>
        )}
        {owners.map((u) => (
          <span
          ...
          >
            ...
            {label(u)}
            {u === installerId && (
              <span className="text-[10.5px] text-faint">· {tr("installer")}</span>
            )}
            {editable && (
              <button
                className={XBTN}
                title={tr("Remove approval owner")}
                onClick={() => remove(u)}
              >
                ×
              </button>
        ...
        {editable && (
          <PersonPicker
            ...
            buttonLabel={`＋ ${tr("Add owner")}`}
            testId="add-approval-owner"
          />
        )}
        {!editable && owners.length > 0 && (
          <span className="text-[11.5px] text-faint">
            {tr("Set by the workspace installer.")}
          </span>
        )}
...
function WaitingRow({ m, onChanged }: { m: ParkedMessage; onChanged: () => void }) {
  ...
  return (
    <div className={ROW + " bg-warnSoft/25"} data-testid={`waiting-${m.id}`}>
      <span className={LABEL}>{tr("Waiting")}</span>
      <span className="min-w-0 flex-1">
        <span className="font-medium text-[13px]">{m.user_name || m.user_id}</span>{" "}
        <span className="text-[12.5px] text-muted">
          {tr("in {place}", { place: m.chat_name || m.chat_id })}
        </span>
      ...
      <button
        ...
        title={tr("Allow the sender and deliver this message now")}
      ...
      <button
        ...
        title={tr("Allow the sender; discard this message")}
        onClick={() => act("allow")}
      >
        {tr("Allow")}
      </button>
...
function ListeningRows({ subs, onChanged }: { subs: Subscription[]; onChanged: () => void }) {
  ...
  return (
    <div className={ROW} data-testid="listening-slack">
      <span className={LABEL}>{tr("Listening")}</span>
