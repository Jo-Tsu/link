    ...
    <div className={GRP + " mt-6"}>
      <details>
        <summary className={ROW + " cursor-pointer hover:bg-paper/60 list-none [&::-webkit-details-marker]:hidden"}>
          <span className="text-[12.5px] text-muted w-24 shrink-0">› {tr("Tools")}</span>
          <span className="min-w-0 flex-1 text-[12.5px] text-muted">
            {tr("{enabled} of {total} enabled", { enabled, total: c.tools.length })}
          </span>
        </summary>
        {c.tools.map((tool) => (
          <label key={tool.name} className={ROW + " cursor-pointer"} title={`${tool.name} — ${tr(tool.description)}`}>
            <input
              type="checkbox"
            ...
            />
            <span className="min-w-0 flex-1 text-[13px] font-medium">{tr(tool.label)}</span>
            <span className={tool.kind === "write" ? TAG_WARN : TAG_QUIET}>
              {tr(tool.kind === "write" ? "asks first" : "read")}
            </span>
          </label>
        ))}
      </details>
