  ...
  return (
    <div data-testid="accounts-detail">
      <div className="flex items-center gap-3.5 mb-5">
        ...
        >
          {busy ? tr("Check your browser…") : `＋ ${tr("Add account")}`}
        </button>
      </div>
      ...
      {(showManual || !c.connected) && (
        <>
          <div className={GRP_H + (accounts.length ? "" : " !mt-0")}>
            {tr(c.managed ? "Add manually" : "Add an account")}
          </div>
      ...
      <div className={FOOT + " mt-2"}>
        {tr("Each account stays separate. Tool results and approvals identify the account used.")}
      </div>
    </div>
  );
}
