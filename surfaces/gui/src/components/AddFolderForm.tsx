  ...
  if (!open) {
    return (
      <button className={"addfolder-trigger" + (compact ? " compact" : "")} onClick={() => setOpen(true)}>
        <Icon name="folderPlus" size={15} /> {tr("Give access to a folder")}
      </button>
    );
  }
    ...
    <div className="addfolder-form">
      <div className="addfolder-row">
        <input
          ...
          autoFocus
          placeholder={tr("Choose or paste a folder path…")}
          value={path}
          spellCheck={false}
        ...
        />
        <button className="btn icon-only" onClick={browse} title={tr("Choose location")} aria-label={tr("Choose location")}>
          <Icon name="folder" size={15} />
        </button>
      </div>
      <div className="addfolder-actions">
        <label className="addfolder-write" title={tr("Off = read-only. Tick to let the agent write here.")}>
