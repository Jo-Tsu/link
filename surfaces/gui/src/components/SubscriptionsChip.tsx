  ...
  return (
    <div className="relative flex-1 min-w-0" ref={wrap}>
      <input
        ...
        placeholder={tr("slack:C0123 or channel link")}
        value={display}
      ...
      {open && (options.length > 0 || lookups.length > 0 || searching) && (
        <div
          className="absolute left-0 right-0 top-full mt-1 z-40 rounded-lg border border-line bg-panel shadow-lg py-1 max-h-56 overflow-y-auto"
          role="listbox"
        ...
        >
          ...
          {searching && lookups.length === 0 && (
            ...
            >
              {tr("Searching workspace channels…")}
            </div>
          )}
            ...
            >
              ...
              {!r.is_member && (
                <span className="block text-[11px] text-warnInk">
                  {tr("Invite @link to this Slack channel before enabling listening.")}
                </span>
              )}
            </button>
          ))}
  ...
  return (
    <div className="sub-chip-wrap" ref={ref}>
      <button
        ...
        title={tr("Channels this session listens to")}
        onClick={() => setOpen((v) => !v)}
      >
      ...
      {open && (
        <div className="sub-pop" onMouseDown={(e) => e.stopPropagation()}>
          <div className="sub-pop-head">{tr("Channels this session listens to")}</div>
          {channels.length === 0 ? (
            <div className="dim sub-pop-empty">{tr("Not subscribed to any channel.")}</div>
          ) : (
            channels.map((c) => {
