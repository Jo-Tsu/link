...
it("rejects non-success REST responses before callers consume error JSON as data", async () => {
  vi.stubGlobal(
    "fetch",
    vi.fn(async () => new Response(JSON.stringify({ detail: "missing or invalid token" }), {
      status: 401,
