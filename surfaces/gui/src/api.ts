import type { Project, SessionInfo, WsEvent } from "./types";

declare const __LINK_DEV_TOKEN__: string;

// Endpoint resolution order: runtime-injected globals (Tauri sets `window.__LINK_HTTP__`
// for its dynamically-chosen sidecar port) → Vite env → the 127.0.0.1:42871 dev default. This
// keeps a single codebase: browser `npm run dev` hits 42871; the desktop shell hits its sidecar.
const httpBase = (): string =>
  (globalThis as any).__LINK_HTTP__ ||
  (import.meta as any).env?.VITE_LINK_HTTP ||
  "http://127.0.0.1:42871";
const wsBase = (): string =>
  (globalThis as any).__LINK_WS__ ||
  (import.meta as any).env?.VITE_LINK_WS ||
  "ws://127.0.0.1:42871";
const apiToken = (): string =>
  (globalThis as any).__LINK_API_TOKEN__ ||
  (import.meta as any).env?.VITE_LINK_API_TOKEN ||
  (typeof __LINK_DEV_TOKEN__ === "string" ? __LINK_DEV_TOKEN__ : "");
...
async function responseError(response: Response): Promise<ApiError> {
  ...
  const message = typeof detail === "string" && detail.trim()
    ? detail
    : `Smallink service request failed (${response.status})`;
...
const fetch = async (
  ...
  if (token) headers.set("X-Link-Token", token);
  const response = await globalThis.fetch(input, { ...init, headers });
  // Some unit tests use a minimal Response-shaped object with no `ok`; only an explicit false
  // means failure. Real browser Responses always provide it.
  if (response.ok === false) {
    if (response.status === 401) {
      window.dispatchEvent(new CustomEvent("smallink:authentication-required"));
    }
...
/** Ask the local sidecar to open a native .zip picker for browser-based Skill imports. */
export async function pickSkillArchiveViaServer(): Promise<string | null> {
  try {
    const res = await fetch(`${httpBase()}/v1/skills/pick-archive`, { method: "POST" });
...
export async function updateProject(
  projectId: string,
  body: Partial<Pick<Project, "name" | "icon" | "description" | "status" | "default_agent" | "default_model" | "pinned" | "sort_order">>,
): Promise<{ ok: boolean; project?: Project; error?: string }> {
  const res = await fetch(`${httpBase()}/v1/projects/${encodeURIComponent(projectId)}`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(body),
  });
...
export async function getProjectSessions(projectId: string): Promise<SessionInfo[]> {
  const res = await fetch(`${httpBase()}/v1/projects/${encodeURIComponent(projectId)}/sessions`);
...
// -- durable task / multi-agent runtime ---------------------------------------
...
export interface RuntimeTaskDetail extends RuntimeTask {
  runs: RuntimeTaskRun[];
}

export interface RuntimeTaskRunDetail extends RuntimeTaskRun {
  agent_runs: RuntimeAgentRun[];
}
...
export async function getAgentCollaborations(limit = 100): Promise<AgentCollaboration[]> {
  const res = await fetch(`${httpBase()}/v1/agent-collaborations?limit=${limit}`);
  if (!res.ok) throw new Error("Could not load agent collaborations");
  return (await res.json()).collaborations ?? [];
}

export async function getSensoryRecords(params: {
  limit?: number;
  offset?: number;
  source_type?: string;
  governance_status?: string;
...
export async function getRuntimeTask(taskId: string): Promise<RuntimeTaskDetail> {
  const res = await fetch(`${httpBase()}/v1/tasks/${encodeURIComponent(taskId)}`);
  if (!res.ok) throw new Error("Task not found");
  return res.json();
}

export async function getRuntimeTaskRun(taskRunId: string): Promise<RuntimeTaskRunDetail> {
  const res = await fetch(`${httpBase()}/v1/task-runs/${encodeURIComponent(taskRunId)}`);
  if (!res.ok) throw new Error("Task run not found");
  return res.json();
}

export async function getRuntimeAgentEvents(agentRunId: string): Promise<RuntimeRunEvent[]> {
  const res = await fetch(
    `${httpBase()}/v1/agent-runs/${encodeURIComponent(agentRunId)}/events`,
  );
  if (!res.ok) throw new Error("Agent run not found");
...
export interface MessageSource {
  ...
  channel_name: string; // resolved; may equal the id (e.g. "#link-test")
  sender_id: string;
...
// -- session roots (orphan Link: scratch + added folders) -------------------
...
export interface Connector {
  ...
  // Local-app connectors (MineM): passive runtime health from the desktop.
  health?: "running" | "offline" | string;
  app_installed?: boolean;
  cli_available?: boolean;
  app_version?: string;
  api_version?: string | number | null;
  visible_asset_count?: number | null;
}

// --- Link Cloud (optional sign-in; manual token paste always works) ---
...
// Pull local Codex conversation sessions into the raw source pool. Reads the folder granted at
...
export interface ModelSettings {
  ...
  // Per-model purpose tags (chat/memory/title) and the available purpose options. Optional so
  // the GUI is robust to an older backend.
  model_purposes?: Record<string, string[]>;
  purposes?: string[];
...
export const CLOUD_CHANGED = "link:cloud-changed";
...
export const INBOX_UNLOCK = "link:inbox-unlock";
...
export const PERSONAS_CHANGED = "link:personas-changed";
...
// Solo page for one gallery link. `capabilities` is the desktop's own
...
// An effective-enabled connector for a session, with a short human detail (e.g. "#link-test · DMs").
...
export interface Subscription {
  ...
  agent: string;
  channel: string;
  channel_name?: string | null; // resolved display name ("link-test"); address stays the id
...
export interface RecentChannel {
  channel: string;
  name?: string | null; // resolved display name, e.g. "link-test" (falls back to the address)
...
// -- Skill Hub ----------------------------------------------------------------
export interface SkillResourceCounts {
  scripts: number;
  references: number;
  assets: number;
  other: number;
...
export interface SkillCatalogItem {
  ...
  source: string;
  source_label: string;
  scope: "builtin" | "user" | "project" | string;
  category: string;
  tags: string[];
  active: boolean;
...
async function skillMutation(
  ...
  if (!res.ok) {
    throw new Error(payload.detail || payload.error || `Skill request failed (${res.status})`);
  }
  return payload;
}

export function createSkill(request: CreateSkillRequest): Promise<SkillDetail> {
  return skillMutation("/v1/skills", request as unknown as Record<string, unknown>);
}

export function importSkill(request: {
  path: string;
  scope: "user" | "project";
  workspace?: string;
}): Promise<SkillDetail> {
  return skillMutation("/v1/skills/import", request);
}

export async function getSkills(workspace?: string): Promise<SkillCatalogResponse> {
  const query = workspace ? `?workspace=${encodeURIComponent(workspace)}` : "";
  const res = await fetch(`${httpBase()}/v1/skills${query}`);
  if (!res.ok) throw new Error(`Skill catalog request failed (${res.status})`);
  return res.json();
}
...
export async function getSkillDetail(
  ...
  const query = workspace ? `?workspace=${encodeURIComponent(workspace)}` : "";
  const res = await fetch(
    `${httpBase()}/v1/skills/${encodeURIComponent(skillId)}${query}`,
  );
...
// Tag a model with zero or more purposes (chat/memory/title). Multi-select; replaces the
// model's current tag set.
export async function setModelPurposes(
  model: string,
  purposes: string[],
): Promise<ModelSettings & { ok: boolean; error?: string }> {
  const res = await fetch(`${httpBase()}/v1/settings/model-purposes`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ model, purposes }),
  });
  return res.json();
}
...
export const AUTOMATIONS_CHANGED = "link:automations-changed";
...
// bot is a member (Slack API constraint); is_member=false → "invite @Link" hint.
...
export class Session {
   ...
   * exactly what the user sees — immune to set_model races across reconnects (a new link
...
// -- Prompts introspection (Memory > System Prompts) ----------------------------
...
export async function getPrompts(): Promise<PromptsResponse> {
  const res = await fetch(`${httpBase()}/v1/prompts`);
  if (!res.ok) throw new Error("Could not load system prompts");
  return res.json();
}
