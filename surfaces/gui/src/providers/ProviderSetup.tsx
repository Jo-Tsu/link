...
export function relTime(epoch?: number | null, tr: Translator = (text, params) => {
  ...
  if (mins < 60) return tr("{count}m ago", { count: mins });
  const hrs = Math.floor(mins / 60);
  if (hrs < 48) return tr("{count}h ago", { count: hrs });
  return tr("{count}d ago", { count: Math.floor(hrs / 24) });
}
...
export function useProviderSetup(opts?: { onSaved?: () => void }): ProviderSetupState {
  ...
  const runTestAndSave = async (): Promise<boolean> => {
    ...
    if (dirty || !info?.configured) {
      const saved = await setProvider(sel, fields).catch(() => ({ ok: false, error: "unreachable" }));
      if (!saved.ok) {
  ...
  const saveField = async (key: string) => {
    ...
    if (!res.ok) {
      setVerify({ state: "error", msg: "couldn't save this field" });
      return;
    }
    await refreshProviders();
    opts?.onSaved?.();
  ...
  const removeKey = async () => {
    if (!sel) return;
    const removed = await removeProvider(sel).catch(() => ({ ok: false, error: "unreachable" }));
    if (!removed.ok) {
      setVerify({ state: "error", msg: removed.error || "couldn't remove provider" });
      return;
    }
    setDrafts((d) => ({ ...d, [sel]: {} }));
    setKeylessOk((s) => {
  ...
  const statusFor = (p: ProviderInfo, o?: { lastUsed?: boolean }) => {
    if (p.configured && p.needs_key) {
      const used = o?.lastUsed ? relTime(p.last_used_at, tr) : null;
      return (
        <span className="block text-[11.5px] text-ok font-medium truncate">
          ✓ {tr("Connected")}{used ? <span className="text-muted font-normal"> · {tr("used {time}", { time: used })}</span> : ""}
        </span>
      );
    }
    if (!p.needs_key)
      return (
        <span className="block text-[11.5px] text-faint truncate">
          {keylessOk.has(p.name)
            ? <span className="text-ok font-medium">✓ {tr("Running")}</span>
            : tr("No key needed")}
        </span>
      );
    return <span className="block text-[11.5px] text-faint truncate">{tr("Not set up")}</span>;
  };
...
export function ProviderCards({
  ...
  const card =
    "flex items-center gap-2.5 rounded-lg border border-line bg-panel px-3 py-2.5 text-left hover:border-lineStrong transition-colors";
  return (
...
export function ProviderForm({
  ...
  return (
    <div>
      ...
      {info?.blurb && <p className="text-[11.5px] text-faint mt-1">{tr(info.blurb)}</p>}
            ...
            <div className="flex gap-2">
              ...
              {testable && (
                <button
                  ...
                  data-testid={`${tp}-test`}
                >
                  {ps.verify.state === "testing" ? "…" : tr(info?.needs_key ? "Test" : "Detect")}
                </button>
              )}
            </div>
            {f.help && !f.secret && <p className="text-[11.5px] text-faint mt-1">{tr(f.help)}</p>}
          </div>
        );
      })}
      ...
      {info?.needs_key && KEY_HELP[sel] && (
        <p className="text-[11.5px] text-faint mt-2">
          ...
          >
            {tr("Create one at {site}", { site: KEY_HELP[sel].label })} ↗
          </button>{" "}
          {tr("— takes about a minute.")}
        </p>
      )}
      {info && !info.needs_key && (
        <p className="text-[11.5px] text-faint mt-2">
          {tr("No API key is needed. Ollama runs models on this Mac.")}{" "}
          <button
            className="text-muted underline decoration-line underline-offset-2 hover:text-ink"
      ...
      {(() => {
        ...
        return (
          <div className="mt-4">
            <label className={label}>{tr(ep.label)}</label>
            ...
            {ep.help && <p className="text-[11.5px] text-faint mt-1">{tr(ep.help)}</p>}
          </div>
        );
      })()}
