...
export function App() {
  ...
  const resumeLastOrGate = async () => {
    ...
    try {
      ...
      setProjects(recents);
      // Only auto-adopt a recent folder for gated surfaces (Code). Link starts orphan.
      ...
      switch (ev.type) {
        ...
        case "turn_end":
          if (d.status === "max_iterations_exceeded")
            setItems((p) => [...p, { kind: "notice", tone: "warn", text: tr("Stopped: max iterations reached.") }]);
          break;
        case "model_changed":
          ...
          if (d.model) setModel(d.model);
          setItems((p) => [...p, { kind: "notice", tone: "info", text: d.text || tr("Model switched") }]);
          break;
        case "interrupted":
          flushPartialStream();
          setItems((p) => [...p, { kind: "notice", tone: "warn", text: tr("Interrupted.") }]);
          break;
        case "error":
          flushPartialStream();
          setItems((p) => [
