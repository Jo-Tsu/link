//! Smallink desktop shell.
//!
//! Tauri is a thin native window over the existing React SPA. It:
//!   1. picks a free localhost port and starts the Python `smallink-server` as a managed
//!      sidecar on that port (so it never clashes with a hand-run server on 42871);
//!   2. injects the sidecar HTTP/WS addresses and per-launch authentication token before the
//!      SPA loads (single codebase — the browser build still hits 42871);
//!   3. lives in the system tray: closing the window hides it (keeps Smallink + the scheduler
...
//! comes from the SecretStore (Settings tab), see `smallink.providers.resolve_api_key`.
...
use smallink_stt::{Dictation, DownloadProgress};
use serde::Serialize;
use tauri::{
    menu::{Menu, MenuItem},
...
/// Native tray labels follow the language selected inside the React client.
...
///   1. `LINK_SERVER_BIN` env override.
...
///   3. Legacy onefile slot: `link-server[.exe]` next to the app binary (pre-onedir
///      builds used Tauri externalBin).
///   4. Dev fallback: the repo venv, relative to this crate (`src-tauri` → repo `.venv`;
///      `bin/` on POSIX, `Scripts\` on Windows).
fn server_bin() -> PathBuf {
    if let Ok(p) = std::env::var("LINK_SERVER_BIN") {
    ...
    if let Ok(exe) = std::env::current_exe() {
        if let Some(dir) = exe.parent() {
            ...
            candidates.push(dir.join("sidecar").join(legacy_name));
            if let Some(contents) = dir.parent() {
                candidates.push(
                    contents
                        .join("Resources")
            ...
            candidates.push(dir.join(legacy_name));
            for c in candidates {
                if c.exists() {
                    return c;
                }
    ...
    if cfg!(windows) {
        p.push("../../../.venv/Scripts/smallink-server.exe");
    } else {
        p.push("../../../.venv/bin/smallink-server");
    }
    p
}

/// Mirror of `smallink.secrets.state_dir()` so the shell and server agree on `desktop.json`.
/// Windows: `%APPDATA%\link`; POSIX: `~/.config/link`. `LINK_STATE_DIR` overrides.
fn state_dir() -> PathBuf {
    if let Ok(d) = std::env::var("LINK_STATE_DIR") {
        return PathBuf::from(d);
    }
    #[cfg(windows)]
    {
        if let Ok(appdata) = std::env::var("APPDATA") {
            return PathBuf::from(appdata).join("link");
        }
    }
    let home = std::env::var("HOME").unwrap_or_else(|_| ".".into());
    PathBuf::from(home).join(".config").join("link")
}
...
/// The sidecar's log file: `<state_dir>/logs/smallink-server.log`, fresh per
...
fn server_log_file() -> Option<std::fs::File> {
    ...
    let path = dir.join("smallink-server.log");
    if path.exists() {
        let _ = std::fs::rename(&path, dir.join("smallink-server.log.old"));
    }
...
/// Native file picker restricted to packaged Skill archives.
#[tauri::command]
async fn pick_skill_archive(app: tauri::AppHandle) -> Option<String> {
    use tauri_plugin_dialog::DialogExt;
    let (tx, rx) = std::sync::mpsc::channel();
    app.dialog()
        .file()
...
// -- local dictation ---------------------------------------------------------------------------
// The actual microphone/model code lives in the Tauri-free `smallink-stt` crate. This shell owns the
...
#[tauri::command]
fn set_app_language(language: String, menu: tauri::State<NativeMenuItems>) -> bool {
    let zh = language.to_ascii_lowercase().starts_with("zh");
    let labels = if zh {
        ("打开 Smallink", "设置", "退出")
    } else {
        ("Open Smallink", "Settings", "Quit")
    };
    menu.open.set_text(labels.0).is_ok()
        && menu.settings.set_text(labels.1).is_ok()
        && menu.quit.set_text(labels.2).is_ok()
}
...
pub fn run() {
    ...
    let inject = format!(
        "window.__LINK_HTTP__={http:?};window.__LINK_WS__={ws:?};window.__LINK_API_TOKEN__={api_token:?};window.__LINK_PLATFORM__={:?};",
        std::env::consts::OS
    );
    ...
    tauri::Builder::default()
        ...
        // duplicate sidecar — which previously left a window stuck on "Starting Smallink…".
        ...
        .setup(move |app| {
            ...
            server_cmd
                ...
                .env("LINK_EXIT_WITH_PARENT", "1")
                .env("LINK_PARENT_PID", std::process::id().to_string())
                .env("LINK_API_TOKEN", &api_token)
                ...
                // (the "Starting Smallink…" freeze on Windows). Hand it real handles: the
            ...
            let child = match server_cmd.spawn() {
                Ok(child) => Some(child),
                Err(e) => {
                    eprintln!("[smallink] failed to start server sidecar: {e}");
                    None
                }
            };
            ...
            let open_i = MenuItem::with_id(app, "open", "Open Smallink", true, None::<&str>)?;
            ...
            // Use the Smallink brand mark in the menu bar as well as the app bundle. This PNG is
            // generated from the same icon master during packaging, so all native surfaces stay
            // visually consistent.
            let tray_icon =
            ...
            TrayIconBuilder::new()
                ...
                .on_menu_event(|app, event| match event.id.as_ref() {
                    ...
                    "settings" => {
                        ...
                        if let Some(w) = app.get_webview_window("main") {
                            let _ = w.eval(
                                "window.dispatchEvent(new CustomEvent('link:open-settings'))",
                            );
                        }
                    }
                    "quit" => app.exit(0),
        ...
        .expect("error while building the Smallink desktop app")
        .run(|app, event| {
            // macOS emits Reopen when the user clicks the Dock icon while the application is
            // already running. Because close-to-tray hides the only window, handling this event
            // explicitly is what makes every Dock click restore and focus Smallink.
            #[cfg(target_os = "macos")]
