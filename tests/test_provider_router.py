...
from smallink.providers.registry import _normalize_ollama_url, build_provider_client
from smallink.providers.openai_provider import _salvage_tool_calls_from_text
...
def _patch_build(monkeypatch):
    ...
    monkeypatch.setattr("smallink.providers.router.build_provider_client", fake_build)
    return state
...
def test_manager_curated_models(tmp_path, monkeypatch):
    ...
    """
    monkeypatch.setenv("LINK_STATE_DIR", str(tmp_path / "state"))
    from smallink.providers.registry import provider_descriptors
...
def test_provider_builders(monkeypatch):
    ...
    from smallink.providers import AnthropicProvider, GeminiProvider
    from smallink.providers.registry import build_provider_client
...
def test_surface_visibility(tmp_path, monkeypatch):
    ...
    assert s == {"link": True, "chat": False, "code": False}
    ...
    # link is always on regardless
    assert mgr.get_settings()["surfaces"]["link"] is True
