...
from smallink.tools.subagent import build_explorer_engine, explorer_tools
...
def test_explore_records_child_agent_run(tmp_path):
    ...
    provider = ScriptedProvider([_text_turn("The report.")])
    reg = ToolRegistry()
    reg.register_all(
        explorer_tools(
            workspace=tmp_path,
            provider=provider,
            model="gpt-5.5",
            run_observer=AgentRunObserver(store, "session-explore"),
        )
    )

    result = reg.execute("explore", {"task": "find the implementation"})
    assert result["report"] == "The report."
    runs = store.list_agent_runs(task_run.task_run_id)
    assert len(runs) == 2
    child = next(run for run in runs if run.agent_role == "explorer")
    assert child.parent_agent_run_id == root.agent_run_id
    assert child.root_agent_run_id == root.agent_run_id
    assert child.status == "completed"
    assert child.output["report"] == "The report."
    assert [event.event_type for event in store.list_events(child.agent_run_id)] == [
        "turn_start",
        "assistant_message",
        "turn_end",
    ]
...
def test_code_engine_registers_explore_chat_does_not(tmp_path):
    ...
    from smallink.agents import code_agent
    from smallink.agents.chat import chat_agent
