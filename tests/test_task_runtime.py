...
def test_runtime_store_repairs_legacy_workspace_project_ids(tmp_path):
    database = tmp_path / "link.db"
    conversations = ConversationStore(tmp_path)
    workspace = str(tmp_path / "workspace")
    conversations.create_project(
        ProjectRecord(project_id="project-uuid", name="Project", workspace_path=workspace)
    )
    conversations.save(
        SessionRecord(
    ...
    runtime = SQLiteTaskRuntimeStore(database)
    runtime.start_root_run(
        session_id="legacy-runtime",
        title="Legacy task",
        trigger="user",
        agent_role="link",
    ...
    assert runtime.repair_legacy_project_ids() == 1
    assert runtime.get_task_by_session("legacy-runtime").project_id == "project-uuid"
...
async def test_tracked_runtime_uses_project_uuid_not_workspace_path(tmp_path):
    from smallink.events import Event, EventType

    manager = SessionManager(workspace=tmp_path, provider=_Provider())
    created = manager.create_project(
        {"name": "Smallink", "workspace_path": str(tmp_path)}
    )
    project_id = created["project"]["project_id"]
    engine = manager.get_engine("project-runtime", workspace=str(tmp_path), agent="code")

    async def fake_run(_content, source=None):
        yield Event(EventType.ASSISTANT_MESSAGE, {"text": "done"})
        yield Event(EventType.TURN_END, {"status": "completed"})

    engine.run = fake_run
    async for _ in manager.tracked_engine_events(
        "project-runtime", engine, content="Inspect the project"
    ):
        pass

    task = manager.runtime_store.get_task_by_session("project-runtime")
    assert task is not None
    assert task.project_id == project_id
    assert task.project_id != str(tmp_path)
