...
model actually reads the image (`LINK_LIVE_VISION=1`, key read from the SecretStore).
"""
...
from smallink.attachments import build_user_content, content_to_text
...
def test_list_content_message_persists_and_titles(tmp_path):
    from smallink.conversations import ConversationStore, title_from
...
@pytest.mark.skipif(
    os.environ.get("LINK_LIVE_VISION") != "1",
    reason="opt-in: real OpenAI vision call (set LINK_LIVE_VISION=1)",
)
def test_live_vision_model_reads_image():
    from smallink.providers import OpenAIProvider
...
def test_pdf_attachment_invalid_or_oversized_skipped():
    from smallink.attachments import MAX_PDF_CHARS
