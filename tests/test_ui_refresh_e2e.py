...
State is fully isolated: `LINK_STATE_DIR` redirects the SecretStore and the manager's data
...
from smallink.interactions import decode
...
def _approve_button(card):
    """The first action button (`link_0` = Approve) of a mirrored approval card."""
    for block in card.get("blocks") or []:
        if block.get("type") == "actions":
            for el in block.get("elements") or []:
                if el.get("action_id") == "link_0":
                    return el
    return None
...
async def test_ui_refresh_cross_cutting_e2e(fake_slack, tmp_path, monkeypatch):
    ...
    # by what this test writes; the manager's own data dir lives under tmp_path/.link.
    ...
    # machine-global ~/.config/link (so this run cannot mutate the real secrets hash).
