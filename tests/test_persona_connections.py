...
def test_persona_enable_toggle(tmp_path, monkeypatch):
    ...
    assert before["ops"]["enabled"] is False  # ships disabled; only link starts on
    assert before["link"]["enabled"] is True
...
def test_fresh_session_view_uses_persona_hint(tmp_path, monkeypatch):
    ...
    # GUI's persona hint the view resolved to the DEFAULT persona (link) — the owner's
