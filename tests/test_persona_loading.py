...
from smallink.personas.loading import consent_summary
from smallink.personas.manifest import ManifestError, parse_manifest
...
def test_uninstall_default_falls_back_to_link(tmp_path):
    reg = PersonaRegistry(state_path=tmp_path / "personas.json")
    reg.install_from_dir(_persona_dir(tmp_path))
