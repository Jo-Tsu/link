...
Asserts capabilities register, ``expand`` reproduces the Code and Link toolsets exactly
(the equivalence net for the build_tools refactor), context prerequisites are honored
(no shell without an executor, no files without a workspace), and the Code/Link file-tool
...
from smallink.agents.code import CODE_CAPABILITIES, code_agent
from smallink.agents.link import LINK_CAPABILITIES, link_agent
from smallink.catalog import CATALOG, capability, expand, risk_summary
from smallink.risk import RiskClass
...
def test_expand_link_matches_expected(tmp_path):
    tools = expand(LINK_CAPABILITIES, _full_context(tmp_path))
    assert _names(tools) == LINK_TOOLS
...
def test_agents_use_catalog(tmp_path):
    ...
    assert _names(link_agent().build_tools(ctx)) == LINK_TOOLS


def test_file_capability_distinction(tmp_path):
    # Code drops read_file_lines (folded into the windowed reader); Link keeps it (multi-root).
    code = _names(expand(["code_files"], _full_context(tmp_path)))
    link = _names(expand(["files"], _full_context(tmp_path)))
    assert "read_file_lines" not in code
    assert "read_file_lines" in link
    assert "read_file" in code and "read_file" in link
