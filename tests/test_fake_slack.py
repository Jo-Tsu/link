...
from smallink.connectors.config import ConnectorSettings
from smallink.connectors.gateway import Gateway
from smallink.interactions import Button
...
async def test_send_and_send_interactive_recorded(fake_slack):
    ...
    assert [e["action_id"] for e in elements] == ["link_0", "link_1"]


# 4. An `link_*` button click → adapter action handler → gateway interaction handler
...
async def test_real_bolt_dispatches_both_envelope_shapes(fake_slack):
    ...
    try:
        ...
        assert seen["action"]["actions"][0]["action_id"] == "link_0"
...
#    Slack encodes "@link hi"), so parked cards / transcripts never show raw ids. Unresolvable
...
async def test_inbound_rewrites_mention_tokens(fake_slack):
    ...
    fake_slack.add_user("UOCW99", "link", display_name="link")
    fake_slack.add_channel("C1", "general", is_im=False)
    ...
    try:
        ...
        assert got[0].text == "@link hi — ask <@UGHOST99> too"
    finally:
        await adapter.disconnect()
