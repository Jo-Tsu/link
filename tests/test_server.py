...
def test_agents_and_memory_rest(tmp_path):
    ...
    # The picker lists enabled+surfaced personas — a fresh install is link-only
    ...
    blocked = client.post("/v1/memory", json={"content": "prefer pathlib"})
...
def test_skill_create_and_import_rest(tmp_path):
    ...
    created = client.post(
        ...
        json={
            ...
            "description": "Turn meeting notes into follow-up actions.",
            "instructions": "Extract owners, deadlines, and unresolved questions.",
            "display_name": "Meeting Follow-up",
            "scope": "user",
        },
    )
    assert created.status_code == 200
    assert created.json()["name"] == "meeting-followup"
    assert any(
        item["name"] == "meeting-followup"
        for item in client.get("/v1/skills").json()["skills"]
    )
    assert client.post(
        "/v1/skills",
        json={
    ...
    imported = client.post(
        "/v1/skills/import", json={"path": str(source), "scope": "project"}
    )
    assert imported.status_code == 200
    assert imported.json()["scope"] == "project"


def test_disable_persona_archives_its_sessions(tmp_path):
    """Disable = "put this agent and its history away": the persona's real sessions are
    ...
    assert store.load("link-a").archived is False
...
def test_artifact_read_accepts_url_encoded_unicode_path(tmp_path):
    target = tmp_path / "交付" / "老板专属-BD-驾驶舱.md"
    target.parent.mkdir()
    target.write_text("# 老板驾驶舱", encoding="utf-8")
    client = _client(tmp_path, [])

    encoded = "%E4%BA%A4%E4%BB%98/%E8%80%81%E6%9D%BF%E4%B8%93%E5%B1%9E-BD-%E9%A9%BE%E9%A9%B6%E8%88%B1.md"
    result = client.get(
        "/v1/sessions/unknown/artifacts/read", params={"path": encoded}
    ).json()

    assert result["ok"] is True
    assert result["content"] == "# 老板驾驶舱"
...
def test_server_sets_explicit_websocket_frame_limit(tmp_path, monkeypatch):
    ...
    from smallink.server import run as server_run
...
def test_standalone_server_token_file_is_user_only(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("LINK_API_TOKEN", raising=False)
    path = server_run._ensure_api_token(9876)
    try:
        assert path == tmp_path / "agent-state" / "sidecar-9876.token"
        assert path.read_text().strip() == os.environ["LINK_API_TOKEN"]
        assert len(path.read_text().strip()) == 64
        assert (path.stat().st_mode & 0o777) == 0o600
    finally:
        path.unlink(missing_ok=True)
        os.environ.pop("LINK_API_TOKEN", None)
...
def test_cors_rejects_foreign_origin(tmp_path):
    ...
    dev = client.get("/v1/sessions", headers={"Origin": "http://localhost:41737"})
    assert dev.headers.get("access-control-allow-origin") == "http://localhost:41737"
...
def test_sidecar_token_gates_rest_and_websockets(tmp_path, monkeypatch):
    from smallink.mcp.config import global_mcp_path
    from starlette.websockets import WebSocketDisconnect as WSD

    monkeypatch.setenv("LINK_API_TOKEN", "a" * 64)
    manager = SessionManager(workspace=tmp_path, provider=ScriptedProvider([]))
    client = TestClient(create_app(manager))
    ...
    assert client.get(
        "/v1/sessions", headers={"X-Link-Token": "wrong"}
    ).status_code == 401

    headers = {"X-Link-Token": "a" * 64}
    ...
    with client.websocket_connect(
        "/ws/session/authed", subprotocols=["link", "a" * 64]
    ) as ws:
        assert ws.accepted_subprotocol == "link"
        assert ws.receive_json()["type"] == "ready"

    with client.websocket_connect(
        "/ws/events", subprotocols=["link", "a" * 64]
    ) as ws:
        assert ws.accepted_subprotocol == "link"
...
def test_ws_browser_tool_audit_round_trip(tmp_path):
    ...
    with client.websocket_connect("/ws/session/browser-audit?agent=link") as ws:
        assert ws.receive_json()["type"] == "ready"
...
def test_workspace_command_trust_controls_live_engine(tmp_path):
    ...
    (proj / ".link").mkdir(parents=True)
    (proj / ".link" / "config.toml").write_text(
...
def test_always_allow_grants_survive_restart(tmp_path):
    ...
    def _run_turn(client, expect_prompts):
        with client.websocket_connect("/ws/session/grants1?agent=link") as ws:
            assert ws.receive_json()["type"] == "ready"
...
def test_google_one_click_paused_but_manual_alive(tmp_path):
    ...
    for name in ("gmail", "google_calendar", "google_drive"):
        c = connectors[name]
        assert c["managed"] is False and c["managed_paused"] is True
