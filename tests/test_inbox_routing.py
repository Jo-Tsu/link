...
from smallink.inbox_routing import (
    DEFAULT_INBOX,
    InboxRouting,
    deliver,
    resolve_from_reply,
...
def test_route_precedence(tmp_path):
    ...
    r.set_binding("ops", channel="slack", target="#ops-agent")
    ...
    assert r.route_for("s2", "link") == DEFAULT_INBOX
...
def test_deliver_to_channel_embeds_item_id(tmp_path):
    ...
    assert f"[link:{item.id}]" in sent["text"]  # rebrand: emits [link:…] since 2026-07-22
...
def test_inbound_reply_resolves_correct_item(tmp_path):
    ...
    ok = resolve_from_reply(f"approve [link:{item.id}]", store.resolve)
    assert ok is True
...
def test_inbound_freetext_answer_to_question(tmp_path):
    ...
    res = resolve_from_reply(f"us-east-1 [link:{q.id}]", store.resolve)
