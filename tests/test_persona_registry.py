...
from smallink.personas.registry import DEFAULT_PERSONA_ID, PersonaRegistry
...
def test_builtins_present(tmp_path):
    reg = _reg(tmp_path)
    assert {"code", "chat", "link", "ops"} <= set(reg.ids())
...
def test_sidebar_defaults_to_link_only(tmp_path):
    reg = _reg(tmp_path)
    sidebar = reg.sidebar()
...
def test_disable_default_falls_back(tmp_path):
    reg = _reg(tmp_path)
    assert reg.default_id() == DEFAULT_PERSONA_ID  # link
    reg.set_enabled("ops", True)  # another persona must be enabled to fall back to
    reg.set_enabled("link", False)
    # Link off → default resolves to another enabled persona, not link.
    assert reg.default_id() != "link"
