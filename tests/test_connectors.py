...
from smallink.connectors import (
    ConnectorSettings,
    FakeAdapter,
    Gateway,
    MessageEvent,
...
from smallink.connectors.base import SendResult
...
def test_send_message_success(tmp_path):
    ...
    assert tool.__link_schema__["function"]["name"] == "send_message"
...
def test_load_settings_from_secretstore(tmp_path, monkeypatch):
    ...
    settings = __import__(
        "smallink.connectors.config", fromlist=["load_settings"]
...
class _StubProvider:
    ...
    def capabilities(self, _model):  # pragma: no cover
        from smallink.providers.base import ModelCapabilities

        return ModelCapabilities()

    def stream(self, **_kw):  # pragma: no cover
        from smallink.providers.base import StreamChunk

        yield StreamChunk(turn=self.complete())


def test_engine_connector_tools_are_link_scoped(tmp_path):
    from smallink.agent import build_engine
    from smallink.agents import chat_agent, code_agent, link_agent
    ...
    assert "send_message" in link.registry.names()
    assert "browser_read_url" in link.registry.names()
    assert "browser_open_url" in link.registry.names()
    assert "browser_click" in link.registry.names()
    assert "browser_type" in link.registry.names()
    assert "github_search" not in link.registry.names()

    # §36: browser READS (registry kind) are free; interactions still gate.
    assert link.registry.get("browser_open_url").metadata.requires_approval is False
    assert link.registry.get("browser_snapshot").metadata.requires_approval is False
    assert link.registry.get("browser_click").metadata.requires_approval is True
    assert link.registry.get("browser_type").metadata.requires_approval is True
    link.permissions.allow_tool_for_session("browser_click")
    decision = link.permissions.evaluate(
        "browser_click",
        {"target": "button"},
        link.registry.get("browser_click").metadata,
    )
    assert decision.needs_user is True

    secrets.put("github:default", {"token": "ghp_test", "enabled": True})
    link_with_github = build_engine(
        agent=link_agent(),
        workspace=tmp_path,
        provider=_StubProvider(),
        secrets=secrets,
    )
    assert "github_search" in link_with_github.registry.names()
    # §36: github_search is a registry READ — free; the write sibling still gates.
    assert (
        link_with_github.registry.get("github_search").metadata.requires_approval
        is False
    )
    assert (
        link_with_github.registry.get(
            "github_create_issue"
...
def test_connector_list_descriptors(tmp_path):
    from smallink.connectors import connector_list
...
def test_connector_list_pre_connect_copy(tmp_path):
    ...
    from smallink.connectors import connector_list
    from smallink.connectors.catalog_copy import ACCESS
    from smallink.connectors.descriptors import list_descriptors
...
def test_reconnect_does_not_clobber_secret_or_allowlist(tmp_path):
    ...
    from smallink.connectors import connect_connector
    from smallink.connectors.descriptors import get_descriptor
...
def test_connect_validation_runs(tmp_path):
    from smallink.connectors import connect_connector
    from smallink.connectors.descriptors import ValidationResult, get_descriptor
...
def test_telegram_message_mapper():
    ...
    from smallink.connectors import telegram_message_to_event
...
def test_slack_event_mapper_and_loop_guard():
    from smallink.connectors import slack_event_to_event
...
def test_make_adapter():
    from smallink.connectors import SlackAdapter, TelegramAdapter, make_adapter
...
async def test_slack_resolves_and_caches_display_name():
    from smallink.connectors import SlackAdapter
...
async def test_slack_resolve_channel_name():
    ...
    class _Client:
        async def conversations_info(self, channel):
            ...
            return {"channel": {"id": channel, "name": "link-test"}}
    ...
    a = SlackAdapter("b", "x")
    a._app = _App()
    assert await a._channel_name("C1") == "link-test"
    assert await a._channel_name("C1") == "link-test"  # served from cache
    assert calls == ["C1"]  # only one API round-trip
    # public §2.1 wrapper delegates to the cached resolver (no extra call)
    assert await a.resolve_channel_name("C1") == "link-test"
    assert calls == ["C1"]
...
def test_new_connectors_connect_and_gate_tools(tmp_path):
    from smallink.connectors import connect_connector, connector_list
    from smallink.connectors.integration_tools import make_integration_tools
...
def _connected_tools(tmp_path, monkeypatch, calls):
    ...
    import smallink.connectors.integration_tools as it
...
def test_registry_has_no_duplicate_names():
    ...
    from smallink.connectors.descriptors import DESCRIPTORS
    from smallink.connectors.tool_defs import TOOL_DEFS
...
def test_managed_callback_profile_keys_by_account_id(tmp_path):
    ...
    from smallink.cloud import managed_profile_from_callback
    from smallink.connectors import accounts
    from smallink.connectors.setup import managed_connect_connector
...
def test_google_drive_multi_account_keys_by_email(tmp_path):
    ...
    assert ids == ["rohit@link.app", "work@acme.com"], ids
...
def test_outlook_managed_multi_account_keys_by_email(tmp_path, monkeypatch):
    ...
    secrets = SecretStore(tmp_path / "secrets.json")
    for email, tok in (("rohit@link.com", "g1"), ("ops@acme.com", "g2")):
    ...
    assert ids == ["ops@acme.com", "rohit@link.com"], ids
    ...
    assert out["account"] == "rohit@link.com"
...
def test_validate_whoami_helper(monkeypatch):
    import smallink.connectors.descriptors as d
...
def test_experimental_hidden_until_enabled(tmp_path, experimental_descriptor):
    from smallink.connectors import connector_list, set_experimental_enabled
...
def test_experimental_package_loads_cleanly():
    ...
    from smallink.connectors.descriptors import DESCRIPTORS
    from smallink.connectors.experimental import EXPERIMENTAL_DESCRIPTORS
