...
from smallink.connectors.tool_defs import TOOL_DEFS, approval_for_tool
from smallink.connectors.tools import make_send_file_tool, make_send_message_tool
...
def test_browser_automation_reads_are_free_interactions_gate():
    from smallink.connectors.browser_automation import make_browser_automation_tools
...
def _fake_roster(monkeypatch, channels_by_team: dict):
    from smallink.connectors import slack_directory
...
def test_bare_channel_names_coerce_to_slack(tmp_path, monkeypatch):
    """The owner's exact transcript: the model sent target='all-link' and
    '#all-link' — no 'slack:' prefix — and got 'invalid target'. Bare names
    ...
    assert tool("all-link", "Hi")["ok"] is True
    assert tool("#all-link", "Hi")["ok"] is True
...
def test_unknown_ambiguous_and_not_member_names_error_actionably(tmp_path, monkeypatch):
    ...
    assert "invite @Smallink" in tool("slack:#private-ops", "Hi")["error"]
