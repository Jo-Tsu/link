...
from smallink.providers import resolve_api_key
...
def test_scratch_base_setting_persists_and_drives_provisioning(tmp_path, monkeypatch):
    ...
    # New installations default to the Smallink workspace root.
    assert client.get("/v1/settings").json()["scratch_base"] == "~/Smallink"

    base = tmp_path / "my agent files"
