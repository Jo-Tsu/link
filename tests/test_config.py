...
def test_global_and_workspace_override(tmp_path):
    ...
    ws = tmp_path / "ws"
    (ws / ".link").mkdir(parents=True)
    (ws / ".link" / "config.toml").write_text(
        'max_iterations = 30\nmode = "plan"\n'
    )
...
def test_workspace_trust_is_canonical_and_user_owned(tmp_path):
    from smallink.workspace_trust import WorkspaceTrustStore
...
def test_build_engine_honors_explicit_empty_command_allowlist(tmp_path):
    ...
    from smallink.config import global_config_path
...
def test_build_engine_respects_max_iterations(tmp_path):
    (tmp_path / ".link").mkdir()
    (tmp_path / ".link" / "config.toml").write_text("max_iterations = 3\n")
...
def test_cloud_endpoints_are_disabled_until_link_owns_a_deployment():
