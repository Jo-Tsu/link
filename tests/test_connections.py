...
from smallink.connections import (
    PersonaConnectionStore,
    SessionConnectionStore,
    effective,
)
from smallink.connectors.base import MessageEvent, SessionSource
from smallink.personas import registry as persona_registry
from smallink.personas.manifest import load_manifest_file
...
def _isolate_state_dir(tmp_path, monkeypatch):
    ...
    global state dir (`~/.config/link`) unless `LINK_STATE_DIR` is set — so without this a
...
def test_muted_connector_tools_absent(tmp_path):
    mgr = SessionManager(workspace=tmp_path, provider=ScriptedProvider())
    # github connected so its tools would otherwise be exposed to a connectors persona (link)
