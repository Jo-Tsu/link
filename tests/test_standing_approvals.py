...
from smallink.automation import Schedule, ScheduledTask, Scheduler, TaskRun, TaskStore
from smallink.automation.models import grant_entries, rule_entry, rule_parts
from smallink.automation.tools import scheduling_tools
from smallink.engine import ApprovalOutcome, PermissionRequest
from smallink.permissions import Mode, PermissionEngine, standing_rule_candidate
...
def test_create_tool_stores_write_grants(tmp_path):
    ...
    props = create.__link_schema__["function"]["parameters"]["properties"]
    ...
    assert (
        "permissions"
        not in update.__link_schema__["function"]["parameters"]["properties"]
    )
...
async def test_scheduled_approver_name_allows_and_denies(tmp_path, monkeypatch):
    ...
    ws.mkdir()
    manager = SessionManager(data_dir=tmp_path / "data", provider=_provider())
    task = _task(workspace=str(ws), agent="link", always_allowed_tools=["web_search"])
    manager.task_store.save(task)
    run = TaskRun(task_id=task.id)
    manager.task_store.add_run(run)
    approver = manager._scheduled_approver(task, run.session_id)
