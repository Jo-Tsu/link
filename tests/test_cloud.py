"""Link Cloud integration: sign-in, managed connect callback, refresh.
...
from smallink.config import Config
from smallink.connectors.setup import (
    connect_connector,
    connector_list,
...
def config():
    return Config(
        ...
        cloud_audience="https://api.cloud.test",
        port=42871,
    )
...
def test_begin_login_builds_pkce_authorize_url(config, monkeypatch):
    monkeypatch.delenv("LINK_PORT", raising=False)
...
def test_managed_connect_redirect_follows_actual_port(secrets, config, monkeypatch):
    ...
    (LINK_PORT), not config.port — the packaged app runs on a random port,
    so an 42871 redirect would hit the wrong (or no) process."""
    ...
    monkeypatch.setenv("LINK_PORT", "52854")  # e.g. what free_port() picked
    ...
    assert out["ok"]
    assert seen["redirect"] == "http://127.0.0.1:52854/oauth/callback"  # not 42871
...
def test_emit_is_content_free_and_hashes_session_id(secrets, config, monkeypatch):
    ...
    body = sent["body"]
    assert body["event"] == "link_session_created"
