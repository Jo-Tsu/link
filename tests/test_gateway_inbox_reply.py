...
An inbound message carrying an `[link:<id>]` token is consumed as an Inbox reply (resolving the
...
from smallink.connectors import ConnectorSettings, FakeAdapter, Gateway, MessageEvent
from smallink.inbox import InboxStore
from smallink.inbox_routing import resolve_from_reply
...
async def test_inbound_reply_resolves_item_and_is_not_routed(tmp_path):
    ...
    await fake.inject(f"approve [link:{item.id}]", user_id="u1")
...
async def test_freetext_answer_to_question_is_consumed(tmp_path):
    ...
    await fake.inject(f"us-west-2 [link:{q.id}]", user_id="anyone")
