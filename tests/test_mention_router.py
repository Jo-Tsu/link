...
@link tagged in a channel is the PRIMARY entry point: with no subscribed session, a
per-thread agent session spawns (visible in the sidebar, thread-scoped standing
send_message grant); follow-up tags in the same thread steer the same session. A
channel with a user-connected (subscribed) agent overrides the router — it must
...
from smallink.connectors.adapters import slack_event_to_event
...
def test_slack_mapper_computes_mentions_me():
    ...
    assert slack_event_to_event(
        {**base, "text": "hey <@UBOT|link> hi"}, "UBOT"
    ).mentions_me
...
def test_mention_spawns_visible_session_with_thread_grant(tmp_path, monkeypatch):
    ...
    engine = mgr._runtimes.engine(sid)
...
def test_subscribed_link_agent_overrides_router(tmp_path, monkeypatch):
    mgr = _mgr(tmp_path)
    captured = _capture_deliveries(mgr, monkeypatch)
    ...
    # Delivered to the connected agent with must-respond framing + the thread target…
...
def test_grant_reseeds_on_engine_rebuild(tmp_path, monkeypatch):
    ...
    stale = mgr._runtimes.discard(sid)  # simulate restart/rebuild
