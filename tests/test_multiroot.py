...
Orphan Link sessions own a primary writable scratch dir and may gain additional folders,
...
from smallink.permissions import Decision, Mode, PermissionEngine
from smallink.providers import AssistantTurn, ToolCall
from smallink.roots import RootDir, normalize_roots, render_context
...
def test_add_root_before_first_turn_persists(tmp_path):
    ...
    the manager creates a minimal link record so the grant isn't lost (GUI start panel).
    """
    mgr = _link_manager(tmp_path)
    ...
    # the grant survived: a fresh manager (no engines) still sees it, under the link agent
    mgr2 = _link_manager(tmp_path)
    persisted = {r["path"]: r for r in mgr2.get_roots(sid)}
    assert str(shared.resolve()) in persisted
    record = mgr2.session_store.load(sid)
    assert record is not None and record.agent == "link"
