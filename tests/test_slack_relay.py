...
from smallink.connectors import relay_client
from smallink.connectors.adapters import make_adapter
from smallink.connectors.base import InteractionEvent, MessageEvent
from smallink.connectors.config import ConnectorSettings, load_settings
from smallink.connectors.relay_client import SlackRelayAdapter
from smallink.connectors.slack_addr import qualify, split
from smallink.connectors.tools import make_send_message_tool
...
async def test_relay_resolves_names_and_mentions(monkeypatch):
    ...
    async def fake_get(team_id, method, params):
        ...
        if method == "users.info":
            names = {
                ...
                "UBOT1": {"profile": {"display_name": "link"}},
            }
            return {"ok": True, "user": names.get(params["user"], {})}
        if method == "conversations.info":
            return {"ok": True, "channel": {"name": "link-test"}}
        return None
    ...
    assert ev.source.chat_name == "link-test"  # not C1
    assert ev.text == "@link hey bot"  # <@UBOT1> rewritten
...
def test_make_adapter_socket_mode_builds_socket_adapter():
    from smallink.connectors.adapters import SlackAdapter
