...
from smallink.tools.files import file_tools
from smallink.tools.git import git_tools
from smallink.tools.search import _py_grep, search_tools
from smallink.web.fetch import _html_to_text, _public_url_error, make_web_fetch_tool
...
def test_ripgrep_uses_the_same_ignored_dirs_as_the_python_fallback(tmp_path, monkeypatch):
    import smallink.tools.search as search
...
def test_web_fetch_rejects_local_and_private_addresses(monkeypatch):
    ...
    assert "local" in (_public_url_error("http://localhost/admin") or "")
    assert "private" in (_public_url_error("https://internal.example") or "")


def test_web_fetch_allows_public_addresses(monkeypatch):
    import socket
    ...
    assert _public_url_error("https://example.com/docs") is None
...
def test_link_has_grep_not_search_files(tmp_path):
    from smallink.agents.base import AgentContext
    from smallink.agents.link import link_tool_factory

    names = {
        getattr(t, "__name__", "")
        for t in link_tool_factory(AgentContext(workspace=tmp_path))
    }
    assert "grep" in names and "search_files" not in names
    assert "git_log" not in names  # git history isn't useful for Link
