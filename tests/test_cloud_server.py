"""Sidecar loopback routes for Link Cloud: /oauth/callback,
...
def test_connect_managed_requires_configured_cloud(client):
...
def test_oauth_callback_writes_profile_and_returns_page(client):
    ...
    assert "Served locally by Smallink" in resp.text
...
def test_delete_persona_refuses_builtin_and_unknown(client):
    body = client.delete("/v1/personas/link").json()
