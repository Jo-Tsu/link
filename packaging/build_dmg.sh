...
#   3. `tauri build --bundles app` → Smallink.app (resources are copied in).
...
# `tauri build` signs the .app + the bundled sidecar with it. Left unset → an ad-hoc signed local
# development app (first launch may still need right-click → Open).
#
...
# NOTARYTOOL_API_KEY_ID / NOTARYTOOL_API_ISSUER_ID — exported, or in $LINK_NOTARY_ENV, or in
# `.link-notary.env` one directory ABOVE the repo (shared by every clone/worktree on a machine,
...
# LINK_SKIP_NOTARIZE=1 to sign but skip the slow notary round-trip. Neither is distributable.
#
# Experimental (use-at-your-own-risk) connectors are EXCLUDED from this build by default —
# the spec strips link.connectors.experimental. Self-builders can opt in with:
#   LINK_EXPERIMENTAL=1 ./build_dmg.sh
set -euo pipefail
...
if [ -n "${APPLE_CERTIFICATE:-}" ] && [ -n "${APPLE_SIGNING_IDENTITY:-}" ]; then
  ...
  KC_DIR="$(mktemp -d)"
  KC="$KC_DIR/link-signing.keychain-db"
...
echo "==> [1/5] PyInstaller: bundling smallink-server ($TRIPLE)"
"$ROOT/.venv/bin/pyinstaller" --noconfirm --clean \
  --distpath "$HERE/dist" --workpath "$HERE/build" "$HERE/smallink-server.spec"

BUILTIN_PERSONA="$HERE/dist/smallink-server/_internal/smallink/personas/builtin/ops.md"
if [[ ! -f "$BUILTIN_PERSONA" ]]; then
  echo "ERROR: bundled sidecar is missing built-in persona manifests: $BUILTIN_PERSONA" >&2
  exit 1
fi
...
rm -rf "$GUI/src-tauri/binaries/sidecar" "$GUI/src-tauri/binaries/smallink-server-$TRIPLE"
...
cp -RL "$HERE/dist/smallink-server" "$GUI/src-tauri/binaries/sidecar"
...
chmod +x "$GUI/src-tauri/binaries/sidecar/smallink-server"
...
if [ -n "${APPLE_SIGNING_IDENTITY:-}" ]; then
  ...
  find "$SIDECAR" -type f ! -name "smallink-server" \
  ...
  codesign --force --sign "$APPLE_SIGNING_IDENTITY" --timestamp --options runtime \
    --entitlements "$GUI/src-tauri/entitlements.plist" "$SIDECAR/smallink-server"
fi
...
# Rust binaries receive a linker-level ad-hoc signature by default on Apple Silicon, but that
# signature does not seal Info.plist or bundled resources. Re-sign the complete app for local
# development builds so the embedded server and resource tree pass macOS bundle verification.
# Production builds continue to use the Developer ID signature applied above/Tauri.
if [ -z "${APPLE_SIGNING_IDENTITY:-}" ]; then
  echo "    applying local ad-hoc signature"
  # Rebuilding over an app that Finder has already opened can leave quarantine,
  # FinderInfo, or resource-fork metadata on bundle files. codesign rejects those
  # attributes before it evaluates the signature, so clear them on every local build.
  xattr -cr "$APP_BUNDLE"
...
if [ "${LINK_SKIP_NOTARIZE:-}" = "1" ] && [ -n "${APPLE_SIGNING_IDENTITY:-}" ]; then
  ...
  echo "==> [5/5] LINK_SKIP_NOTARIZE=1 — signing container, SKIPPING notarize/staple (do not distribute)"
  ...
  NOTARY_ENV="${LINK_NOTARY_ENV:-$ROOT/../.link-notary.env}"
  ...
  if [ -n "${NOTARYTOOL_API_KEY_PATH:-}" ] && [ -n "${NOTARYTOOL_API_KEY_ID:-}" ] \
    ...
    echo "    (env, \$LINK_NOTARY_ENV, or $NOTARY_ENV)."
  fi
else
